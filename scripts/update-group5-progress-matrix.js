...
const { checkPageEvidence } = require('./verify-group5-page-evidence-completeness');

const ROOT = path.join(__dirname, '..');
const PLAN_PATH = path.join(ROOT, 'docs', 'product', 'GROUP5_EXECUTION_PLAN.md');
const MIGRATION_PATH = path.join(ROOT, 'docs', 'product', 'GROUP5_MIGRATION_REPORT.md');
const AUDIT_PATH = path.join(ROOT, 'docs', 'product', 'GROUP5_ACCEPTANCE_AUDIT.md');
const OUT_PATH = path.join(ROOT, 'docs', 'product', 'GROUP5_PROGRESS_MATRIX.md');
const EVIDENCE_ROOT = path.join(ROOT, 'docs', 'product', 'evidence', 'group5');
...
function parsePages(planText) {
  const pages = [];
  const re = /^\d+\.\s+([a-z0-9_-]+)\.html\b/gim;
  let m;
...
function main() {
  ...
  const lines = [];
  lines.push('# GROUP5_PROGRESS_MATRIX');
  lines.push('');
  ...
  console.log(`GROUP5 progress matrix updated: ${path.relative(ROOT, OUT_PATH)}`);
