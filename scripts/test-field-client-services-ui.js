...
(async()=>{
  ...
  await page.locator('#serviceLoad').click();await state('ready');await page.locator('[data-rule=interval]').fill('2');await page.locator('[data-rule=anchorOn]').fill('2032-01-05');await page.locator('#serviceMonth').fill('2032-08');
  await page.reload({waitUntil:'networkidle'});await page.locator('#serviceLoad').click();await state('ready');assert.equal(await page.locator('[data-rule=interval]').inputValue(),'2');assert.equal(await page.locator('[data-rule=anchorOn]').inputValue(),'2032-01-05');
  await page.locator('#servicePreview').click();await state('preview');await page.locator('#serviceConfirm').click();await state('confirmed');
  const recurringPlan=await prisma.clientRatePlan.findFirstOrThrow({where:{clientId:client.id},orderBy:{version:'desc'}});assert.equal(recurringPlan.version,2);assert.equal(recurringPlan.snapshot.servicePlan.seasons[0].schedules[0].interval,2);assert.equal(recurringPlan.snapshot.servicePlan.seasons[0].monthlyCents,15000);
  console.log('PASS recurring editor reference and interval survive reload and save a reviewed version with unchanged monthly price');
