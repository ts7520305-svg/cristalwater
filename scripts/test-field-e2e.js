...
(async () => {
  ...
  try {
    for (const persona of [
      ...
      await context.addInitScript(({token,user,techId,clientId,origin})=>{ if(location.origin!==origin||window.top!==window)return;localStorage.setItem('cristalwater_jwt',token);localStorage.setItem('cristalwater_user',JSON.stringify(user));localStorage.setItem('token',token);localStorage.setItem('user',JSON.stringify(user));if(user.role==='TECHNICIAN')localStorage.setItem('cwTechnicianId',techId);if(user.role==='CLIENT')localStorage.setItem('clientId',clientId); },{token:login.token,user,techId:String(tech.id),clientId:String(client.id),origin:new URL(base).origin});
      ...
      try {
        ...
        if(persona.role==='TECHNICIAN') {
          ...
          await waitBrowserState(page, async id=>(await window.CWFieldPhotos.list(id)).length===1,visit.id);
          await page.locator('#pumpReminderMinutes').fill('1');
          await page.locator('#pumpReminderCreate').click();
          await page.waitForFunction(()=>document.querySelectorAll('#pumpReminderBanner [data-pump-reminder]').length===1);
          assert.equal(await page.locator('#pumpReminderBanner [data-pump-reminder]').count(),1);
          await page.locator('#finishBtn').click();
          await waitBrowserState(page, async id => await window.CWFieldOffline.pending(id), visit.id);
          const pendingBody = await page.evaluate(async id => { const rows = await CWFieldOffline.entries(); const row = rows.find(row => row.resourceId === Number(id)); if(!row)throw Error('Missing pending request for '+id+'; found '+JSON.stringify(rows.map(row=>({id:row.resourceId,requestId:row.requestId}))));return { ...row.payload, requestId: row.requestId }; }, visit.id);
          for(const width of [320,390,1440]) { await page.setViewportSize({width,height:844}); assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),'Pending recovery must fit the viewport'); }
          await page.setViewportSize({width:390,height:844});
          if(process.env.CW_CAPTURE_UI){await page.locator('#cwFieldSyncStatus').scrollIntoViewIfNeeded();await page.screenshot({path:'reports/field-ui/MODERN_WRITE_RECOVERY.png',fullPage:false});}
          ...
          const savedPump=await page.evaluate(()=>{const key=`cwFieldReminders:v1:${CWFieldWriteStore.session().owner}`;return {key,value:localStorage.getItem(key)};});
          await page.evaluate(key=>localStorage.setItem(key,'{invalid'),savedPump.key);
          await page.locator('#dayReviewBtn').click();
          await page.waitForFunction(()=>document.querySelector('#dayReviewResult').textContent.includes('Não considere o dia conferido'));
          assert.equal(await page.evaluate(key=>localStorage.getItem(key),savedPump.key),'{invalid');
          await page.evaluate(({key,value})=>localStorage.setItem(key,value),savedPump);
          ...
          await page.waitForFunction(()=>document.querySelector('#pumpReminderBanner').textContent.includes('envio pendente'));
          assert.match(await page.locator('#pumpReminderBanner').textContent(),/envio pendente/);
          await page.route(`**/api/visits/${visit.id}/photo`,async route=>{const response=await route.fetch(),data=await response.json();await route.fulfill({status:200,json:{...data,sha256:'mismatched-photo-bytes'}});});
          await page.route(`**/api/core/visits/${visit.id}/complete`,async route=>{assert.deepEqual(route.request().postDataJSON(),pendingBody);const response=await route.fetch(),data=await response.json();await route.fulfill({status:200,json:{...data,visit:{...data.visit,id:999999}}});});
          await context.setOffline(false);
          await page.waitForFunction(()=>document.querySelector('#pumpReminderBanner').hidden);
          const pump=await prisma.operationalReminder.findFirst({where:{sourceKey:{startsWith:'pump:'},assignedToTechnicianId:tech.id,metadata:{path:['visitId'],equals:visit.id}}});
          assert(pump?.isCompleted,'Offline pump creation and return to automatic must both reach the server');assert.equal(pump.metadata.visitType,'REGULAR');
          console.log('PASS pump manual reminder survives offline reload and reconciles automatic confirmation');
          await waitBrowserState(page,()=>CWFieldWriteStore.records('VISIT_PHOTO').then(rows=>rows.some(row=>row.failure?.message.includes('fotografia guardada'))));
          assert.equal(await page.evaluate(async id=>(await CWFieldPhotos.list(id))[0].file.size,visit.id),photoBytes.length);
          await page.unroute(`**/api/visits/${visit.id}/photo`);
          if(!await page.locator('#cwFieldSyncStatus details').evaluate(node=>node.open))await page.locator('#cwFieldSyncStatus summary').click();
          await page.locator('#cwFieldSyncStatus button').click();
          await waitBrowserState(page,()=>CWFieldOffline.entries().then(rows=>rows.some(row=>row.failure?.message.includes('ainda não está confirmada'))));
          assert.equal((await prisma.serviceVisit.findUnique({where:{id:visit.id}})).status,'DONE');
          assert.equal(await page.evaluate(id=>CWFieldOffline.pending(id),visit.id),true);
          await page.unroute(`**/api/core/visits/${visit.id}/complete`);await page.locator('#cwFieldSyncStatus button').click();
          await waitBrowserState(page, async id=>!await window.CWFieldOffline.pending(id),visit.id,{timeout:15000});
          console.log('PASS modern real recovery buttons retain mismatched photo/completion acknowledgements and replay original content without duplicate effects');
