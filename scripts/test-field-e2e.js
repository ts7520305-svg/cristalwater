    ...
    for (const persona of [
      ...
      try {
        ...
        if(persona.role==='ADMIN') {
          ...
          await page.locator('#messageInput').fill('Rascunho do cliente A');
          ...
          await page.locator('#messageInput').fill('Rascunho do cliente B');
          releaseOld();
          ...
          assert.equal(await page.locator('#messageInput').inputValue(),'Rascunho do cliente A');
          assert.equal(errors.length,0,errors.join('\n'));
          console.log('PASS admin client switch ignores late responses and isolates/restores drafts');
          await prisma.stockBalance.create({data:{scope:'CENTRAL',productName:'LEGACY TRANSFER UI QA',unit:'L',quantity:10}});
          await page.goto(base+'/admin-inventory',{waitUntil:'networkidle'});await page.locator('#transferVehicleId').selectOption(String(vehicle.id));await page.locator('#transferProductName').selectOption('LEGACY TRANSFER UI QA');assert.equal(await page.locator('#transferUnit').inputValue(),'L');await page.locator('#transferQuantity').fill('2');
          let droppedInventory=false;await page.route('**/api/inventory/transfer-to-vehicle',async route=>{if(!droppedInventory){droppedInventory=true;await route.fetch();await route.abort('failed');}else await route.continue();});
          await page.locator('#transferForm button[type=submit]').click();await page.waitForFunction(()=>document.getElementById('inventoryStatus').classList.contains('error'));await page.locator('#transferForm button[type=submit]').click();await page.waitForFunction(()=>document.getElementById('transferQuantity').value==='');
          ...
          console.log('PASS legacy inventory uses vehicle/product selectors and replays a lost transfer response once');
          ...
          let droppedManual=false;await page.route('**/api/inventory/consume',async route=>{if(!droppedManual){droppedManual=true;await route.fetch();await route.abort('failed');}else await route.continue();});
          await page.locator('#consumeForm button[type=submit]').click();await page.waitForFunction(()=>document.getElementById('inventoryStatus').classList.contains('error'));await page.locator('#consumeForm button[type=submit]').click();await page.waitForFunction(()=>document.getElementById('consumeQuantity').value==='');
          ...
          let droppedEntry=false;await page.route('**/api/inventory/purchases',async route=>{if(!droppedEntry){droppedEntry=true;await route.fetch();await route.abort('failed');}else await route.continue();});
          await page.locator('#purchaseForm button[type=submit]').click();await page.waitForFunction(()=>document.getElementById('inventoryStatus').classList.contains('error'));await page.locator('#purchaseForm button[type=submit]').click();await page.waitForFunction(()=>document.getElementById('supplierName').value==='');
          ...
          console.log('PASS manual consumption and purchase with attachment recover lost responses without duplicate stock');
