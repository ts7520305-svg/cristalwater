...
module.exports=async function createReviewedTechnician(call,input,{response=false}={}){
 ...
 if(reviewed.status!==undefined&&reviewed.status!==200)throw Error('QA technician review failed: '+reviewed.status);
 ...
 if(saved.status!==undefined&&saved.status!==200)throw Error('QA technician commit failed: '+saved.status);
