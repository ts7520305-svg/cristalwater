...
(async () => {
  ...
  assert(r.issues.rows.some(x=>x.lineId&&x.reason==='MAINTENANCE_DECISION_UNCONFIRMED'));assert(r.issues.rows.some(x=>x.lineId&&x.reason==='REPAIR_ORIGIN_UNCONFIRMED'));assert.equal(r.serviceReviewAmountCents-before.serviceReviewAmountCents,2000);
  ...
  r = await read(); assert.equal(r.linkedServiceAmountCents - before.linkedServiceAmountCents, 3000); assert.equal(r.serviceReviewAmountCents - before.serviceReviewAmountCents, 6500); assert(r.issues.rows.some(i => i.reason === 'DUPLICATE_REFERENCE'));
  ...
  await prisma.invoice.update({ where: { id: duplicate.id }, data: { status: 'UNKNOWN' } }); assert.equal((await read()).serviceReviewAmountCents - before.serviceReviewAmountCents, 6500);
  ...
  for (const [patch, reason] of [[{ clientId: other.id }, 'CLIENT_MISMATCH'], [{ clientId: null }, 'CLIENT_MISMATCH'], [{ status: 'PENDING' }, 'SERVICE_NOT_COMPLETED'], [{ endAt: null }, 'MISSING_COMPLETION_DATE'], [{ contractService: { billing: 'INCLUDED_MONTHLY' } }, 'SERVICE_INCLUDED_OR_UNCONFIRMED']]) {
    await prisma.serviceVisit.update({ where: { id: sameId }, data: patch }); r = await read(); assert.equal(r.serviceReviewAmountCents - before.serviceReviewAmountCents, 5000); assert(r.issues.rows.some(i => i.reason === reason), reason);
  ...
  await prisma.extraVisit.update({ where: { id: sameId }, data: { includedInPackage: true } }); assert.equal((await read()).serviceReviewAmountCents - before.serviceReviewAmountCents, 6500); await prisma.extraVisit.update({ where: { id: sameId }, data: { includedInPackage: false } });
  ...
  const malformedAlias = await make([line('SERVICE', 30, { lineType: 'EXTRA_VISIT', referenceId: sameId })], { month: '2005-10' }); r = await read(); assert.equal(r.serviceReviewAmountCents - before.serviceReviewAmountCents, 9500); await prisma.invoice.delete({ where: { id: malformedAlias.id } });
