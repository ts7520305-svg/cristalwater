...
const pages = walk(frontend).filter(file => file.endsWith('.html')).sort().map(file => {
  ...
  const route = '/' + (topLevel ? relative.slice(0, -5) : relative), html = read(file), assets = [], missingAssets = [], guards = [], roles = new Set();
  for (const match of html.matchAll(/<(script|link)\b[^>]*>/gi)) {
    ...
    assets.push(pathname); if (!served && !exists) missingAssets.push(pathname);
...
if (process.argv.includes('--write')) {
  fs.writeFileSync(path.join(root, 'docs/product/PAGE_INVENTORY_20260917.json'), JSON.stringify(result, null, 2) + '\n');
  fs.writeFileSync(path.join(root, 'docs/product/PAGE_INVENTORY_20260917.md'), markdown);
}
