...
(async()=>{
 ...
 await prisma.monthlyReport.createMany({data:[{month:monthRef,type:'ADMIN',data:{}},{month:monthRef,type:'CLIENT',clientId:client.id,data:{}},{month:monthRef,type:'EXTRA_VISITS',clientId:client.id,data:{items:[{visitId:1},{visitId:2}]}},{month:nextMonth,type:'ADMIN',data:{}}]});
 ...
 assert.equal(await metric('extraReports').textContent(),'1');assert.equal(await metric('otherReports').textContent(),'0');assert.equal(await page.locator('#reportsPanel').getAttribute('data-state'),'ready');
