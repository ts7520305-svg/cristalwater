...
(async () => {
  ...
  const compositionCommand = (command, resourceId, data) => { const requestId = randomUUID(); f.requests.push(requestId); return { requestId, command, resourceId, data: { ...data, reason: 'Base e parcela de manutenção conferidas', confirmed: true } }; };
  ...
  // A different command can use this number as an expense ID rather than a basis ID.
  const foreign = await prisma.fieldWriteRequest.create({data:{owner:'ADMIN:'+admin.id,requestId:randomUUID(),scope:'LABOR_COST_COMPOSITION',resourceId:composition.basis.id,payloadHash:'f'.repeat(64),response:{applied:false,code:'QA_UNRELATED_RECEIPT'}}});
  await f.cleanup(); f = null; assert.equal(await prisma.fieldWriteRequest.count({where:{id:foreign.id}}),1); await prisma.fieldWriteRequest.delete({where:{id:foreign.id}});
