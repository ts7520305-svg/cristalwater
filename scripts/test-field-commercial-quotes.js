...
(async () => {
 ...
 const client = await prisma.client.create({data:{name:'Quote client',phone:'PRIVATE-TELEPHONE',active:true,status:'ACTIVE',billingActive:true,monthlyFee:80,monthlyAmount:80}});
 const pool = await prisma.pool.create({data:{name:'Quote pool',clientId:client.id}});
 const repair = await prisma.repair.create({data:{poolId:pool.id,problem:'Substituir bomba',quantity:3}});
 ...
 assert.equal((await call(path+'/schedule',at,'PUT',{})).status,409);
 assert.equal((await call(path,at,'DELETE')).status,409);
 const pdf=await fetch(BASE+path+'/pdf',{headers:{Authorization:`Bearer ${at}`}});assert.equal(pdf.status,200);assert.match(pdf.headers.get('content-type'),/application\/pdf/);
 const bytes=Buffer.from(await pdf.arrayBuffer());assert.equal(bytes.subarray(0,4).toString(),'%PDF');
 if(process.env.CW_QA_PDF_PATH)require('node:fs').writeFileSync(process.env.CW_QA_PDF_PATH,bytes);
 const beforeBill=await call('/api/core/invoices/generate',at,'POST',{clientId:client.id,monthRef:'2034-01'});assert.equal(beforeBill.status,200,JSON.stringify(beforeBill));assert.equal(beforeBill.body.invoice.total,80);
 const oldBill=await call('/api/operational-flow/generate-monthly-invoice',at,'POST',{clientId:client.id,monthRef:'2034-02'});assert.equal(oldBill.status,200,JSON.stringify(oldBill));assert.equal(oldBill.body.invoice.total,80);
 ...
 const savedRepair=await prisma.repair.findUnique({where:{id:repair.id}});assert.equal(savedRepair.totalPrice,230);assert.equal(savedRepair.quantity,3);
 const afterBill=await call('/api/core/invoices/generate',at,'POST',{clientId:client.id,monthRef:'2034-03'});assert.equal(afterBill.status,200,JSON.stringify(afterBill));assert.equal(afterBill.body.invoice.total,310);
