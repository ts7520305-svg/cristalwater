...
function resolvePageRouteTargets(args) {
	...
	if (pages.length === 0 || routes.length === 0) {
		throw new Error('Uso: --page <slug> --route </rota> ou --pages <p1 p2 p3> --routes </r1 /r2 /r3>');
	}
...
async function getAdminSession() {
	const baseUrl = String(process.env.GROUP4_EVIDENCE_BASE_URL || DEFAULT_BASE_URL).trim();
...
function getOutDir(phase, page) {
	return path.join(ROOT, 'docs', 'product', 'evidence', 'group4', phase, page);
}
...
async function run() {
	...
	if (!phase) {
		throw new Error('Uso: node scripts/group4-page-evidence.js --page admin-menu --route /admin-menu --phase before|after');
	}
