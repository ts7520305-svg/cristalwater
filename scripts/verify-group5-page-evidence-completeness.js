...
function checkPageEvidence(page) {
  ...
  for (const phase of ['before', 'after']) {
    ...
    for (const file of req[phase]) {
      ...
      if (!exists(abs)) {
        const msg = `${phase}: ficheiro em falta (${path.join('docs', 'product', 'evidence', 'group5', phase, page, file)})`;
        problems.push(msg);
        missingArtifacts.push(msg);
      }
    }
...
function main() {
  ...
  if (pages.length === 0) {
    console.error('Uso: node scripts/verify-group5-page-evidence-completeness.js --page admin-menu [--json true]');
    console.error('Ou: node scripts/verify-group5-page-evidence-completeness.js --pages admin-a admin-b admin-c [--json true]');
    process.exit(1);
  }
