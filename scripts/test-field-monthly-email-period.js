...
const stamp = 'monthly-' + randomUUID(), month = '2190-02';
...
(async () => {
  ...
  async function api(body, custom = headers) {
    const r = await fetch(base + '/api/admin/reports/send-now', { method: 'POST', headers: custom, body: JSON.stringify(body) });
  ...
  const previousMonth = await prisma.monthlyReport.create({ data: { clientId: clients[0].id, month: '2190-01', type: 'CLIENT', data: { client: 'WRONG-MONTH', pools: [] } } });
  ...
  await prisma.monthlyReport.create({ data: { month, clientId: mismatchClient.id, type: 'CLIENT', data: { ...reports[0].data, clientId: mismatchClient.id, period: { start: '2190-01-01T00:00:00.000Z', end: '2190-02-01T00:00:00.000Z', timeZone: 'UTC' } } } });
  ...
  assert.equal(generated.status, 200, JSON.stringify(generated.body)); assert.equal(generated.body.monthRef, month); assert.equal(generated.body.sent, 0); assert.equal(generated.body.blocked, 'EMAIL_DISABLED'); assert.equal(generated.body.deliveryConfirmed, false); assert.match(generated.cache, /private.*no-store/);
  ...
  try {
    const result = await sendMonthlyReports({ monthRef: month, manual: true });
    ...
    assert(logs.every(l => l.subject.endsWith(month) && l.text.includes('Mês: ' + month) && l.mode === 'MANUAL'));
    assert.deepEqual(await snapshot(), saved);
    // Fault injection at the persistence boundary, confined to this process.
    // All successful writes and the assertions above use the real QA database.
    const logCreate = prisma.emailLog.create, logUpdate = prisma.emailLog.update;
    try {
      prisma.emailLog.create = async () => { throw Error('QA log insertion failed'); };
      const beforeCalls = calls.length, blocked = await sendMonthlyReports({ monthRef: month, manual: true });
      assert.equal(blocked.failed, 3); assert.equal(calls.length, beforeCalls); assert.equal(blocked.sent, 0);
      prisma.emailLog.create = logCreate;
      prisma.emailLog.update = async () => { throw Error('QA log update failed'); };
      const uncertainLog = await sendMonthlyReports({ monthRef: month, manual: true });
      assert.equal(uncertainLog.sent, 1); assert.equal(uncertainLog.logUnconfirmed, 3);
    } finally {
      prisma.emailLog.create = logCreate; prisma.emailLog.update = logUpdate;
    }
    await prisma.notificationRule.update({ where: { id: rule.id }, data: { roles: 'ADMIN' } });
    const beforeCalls = calls.length, adminOnly = await sendMonthlyReports({ monthRef: month, manual: true });
  ...
  console.log('PASS shared client-only entry point, explicit recipient configuration, provider acceptance/rejection/unknown outcomes, saved retry content and write failures without blind retries; zero real emails');
