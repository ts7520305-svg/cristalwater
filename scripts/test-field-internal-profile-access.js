...
(async () => {
  ...
  fs.writeFileSync(dataFile, JSON.stringify([{ id: 15, author: 'ADMIN', text: 'Preserved historical message', created_at: '2026-01-01T00:00:00.000Z' }]));
  for (const [token, role, id] of [[at, 'ADMIN', admin.id], [tt, 'TECHNICIAN', tech.id], [lt, 'TEAM_LEADER', leader.id]]) {
    const sent = await call(chatPath, token, 'POST', { author: role === 'ADMIN' ? 'TECHNICIAN' : 'ADMIN', actorId: 999999, text: '  Real author  ' });
  ...
  const ut = sign({ id: user.id, userId: user.id, technicianId: tech.id, principalType: 'USER', role: 'TECHNICIAN' });
  const linked = await call(chatPath, ut, 'POST', { text: 'Linked identity' }); assert.equal(linked.body.actorType, 'USER'); assert.equal(linked.body.actorId, user.id); assert.equal(linked.body.technicianId, tech.id);
  const many = await Promise.all(Array.from({ length: 12 }, (_, n) => call(chatPath, tt, 'POST', { text: `Concurrent message ${n}` })));
  assert(many.every(row => row.status === 201)); assert.equal(new Set(many.map(row => row.body.id)).size, 12);
  const messages = (await call(chatPath, at)).body; assert.equal(messages.length, 17); assert.equal(messages[0].text, 'Preserved historical message');
  ...
  assert.equal((await call(chatPath, at)).status, 500); assert.equal((await call(chatPath, at, 'POST', { text: 'Must not erase history' })).status, 500); assert.equal(fs.readFileSync(dataFile, 'utf8'), '{corrupt');
  fs.writeFileSync(dataFile, '{}'); assert.equal((await call(chatPath, tt, 'POST', { text: 'Must not replace invalid structure' })).status, 500); assert.equal(fs.readFileSync(dataFile, 'utf8'), '{}');
