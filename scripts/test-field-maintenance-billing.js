...
(async () => {
  ...
  const historicDecision = (await listing()).rows.find(r => r.sourceId === extra.sourceId);
  assert.equal(historicDecision.reviewedFor.clientId, client.id); assert.equal(historicDecision.reviewedFor.clientName, client.name);
  assert.equal(historicDecision.decision.clientId, client.id, 'Saved decisions identify their original customer after a pool transfer');
  ...
  await prisma.pool.update({ where: { id: pool.id }, data: { clientId: otherClient.id } });
  await page.locator('#mbCancel').click(); await page.locator('#mbRefresh').click(); await status.filter({ hasText: 'Intervenções atualizadas.' }).waitFor();
  assert.match(await page.locator('#mbContext').textContent(), /Outro cliente QA/);
  assert.match(await page.locator(`#mbRows [data-mb-source="EQUIPMENT:${extra.sourceId}"]`).textContent(), /Decisão para Cliente de intervenções QA/);
  await prisma.pool.update({ where: { id: pool.id }, data: { clientId: client.id } });
  await page.locator('#mbRefresh').click(); await status.filter({ hasText: 'Intervenções atualizadas.' }).waitFor(); await prepare((await listing()).rows.find(row => row.sourceId === stale.sourceId));
  fs.mkdirSync(output, { recursive: true });
