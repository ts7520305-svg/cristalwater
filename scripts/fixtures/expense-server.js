...
prisma.$transaction = (work, options) => transaction(async db => {
  for (const [model, method, match] of [['expenseAllocation', 'create', 'period-create'], ['expenseEvent', 'create', 'audit'], ['companyExpense', 'update', 'after-payment'], ['companyExpense', 'findMany', 'read'], ['laborCostValuationPart', 'create', 'composition-part'], ['fieldWriteRequest', 'create', 'composition-receipt'], ['userAuditLog', 'create', 'equipment-audit']]) {
...
for (const page of ['admin-expenses', 'admin-ai', 'labor-cost-bases']) app.get('/' + page, (req, res) => res.sendFile(path.resolve(__dirname, '../../frontend/' + page + '.html')));
