...
prisma.$transaction=(work,options)=>transaction(async db=>{
  for(const [model,method,match]of [['revenueEvent','create','audit'],['revenueAllocation','findMany','read']]){const original=db[model][method].bind(db[model]);db[model][method]=(...args)=>{if(fault===match)throw Error('QA_PRIVATE_REVENUE_FAILURE');return original(...args);};}
  return work(db);
},options);
const app=express();app.use(express.json({limit:'32kb'}));app.use('/api/revenue-allocations',require('../../src/routes/monthlyRevenueRoutes'));app.use('/api/ai-admin',require('../../src/routes/aiAdminRoutes'));
for(const page of ['admin-revenue','admin-ai'])app.get('/'+page,(req,res)=>res.sendFile(path.resolve(__dirname,'../../frontend/'+page+'.html')));
