...
(async () => {
  ...
  const contrast = await page.locator('#expensePage main p,#expensePage main h2,#expensePage .metric strong,#expensePage .metric span').evaluateAll(ns => {
    ...
    return ns.filter(n => n.getClientRects().length).map(n => { let p = n, bg; while (p) { bg = getComputedStyle(p).backgroundColor; if (!['transparent', 'rgba(0, 0, 0, 0)'].includes(bg)) break; p = p.parentElement; } const a = light(getComputedStyle(n).color), b = light(bg); return (Math.max(a,b) + .05) / (Math.min(a,b) + .05); });
  }); assert(contrast.every(c => c >= 4.5), JSON.stringify(contrast)); await page.screenshot({ path: path.join(visual, 'expenses-dark-390.png') }); await page.emulateMedia({ colorScheme: 'light' });
