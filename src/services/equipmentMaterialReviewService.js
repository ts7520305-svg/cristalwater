...
async function context(db, completionId, lock = false) {
  ...
  const trusted = materials.intact(ownOriginal, inputs.receipts) && (!original.record || materials.sound(original.record));
...
async function calculate(db, completionId, body, lock = false) {
  ...
  const proposed = { action: body.action, record: input ? materials.create(input, c.visit, c.origin.visitType) : null };
