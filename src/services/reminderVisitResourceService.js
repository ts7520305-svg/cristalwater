...
async function calculate(db,reminderId,body,lock=false){
  ...
  const value={schema:1,basis:rules.basis,reminderId,selection:body,contextHash:c.contextHash,previousHash:c.previousHash,origin,association,parent,peers,movements,comparison,affectedShares,original};return rules.preview({available:true,...value,hash:writes.hash(value)},writes.hash);
}
