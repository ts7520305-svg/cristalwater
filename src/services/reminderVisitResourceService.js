...
async function context(db,reminderId,lock=false){
  ...
  const external=await require('./recordedWorkTimeService').conflicts(db,own.active?.result.event.preview.selection.data.workTime&&parent?[{type:parent.type,id:parent.id,technicianId:parent.technicianId,startAt:new Date(own.active.result.event.preview.selection.data.workTime.startedAt),endAt:new Date(own.active.result.event.preview.selection.data.workTime.endedAt)}]:[]);
...
async function calculate(db,reminderId,body,lock=false){
  ...
  if(body.action==='VOID'){if(!c.journalValid||!active?.canVoid)return refuse('DECLARATION_CHANGED','A declaração mudou ou o comprovativo precisa de revisão.');original=active.result;origin=original.event.preview.origin;}
    ...
    try{comparison=rules.compare(body.data,parent,peers,movements);if(body.data.workTime&&Date.parse(body.data.workTime.endedAt)>Date.parse(association.event.preview.source.completedAt))throw Error('After completion');}
    ...
    if(body.data.workTime){const w=body.data.workTime,conflicts=await require('./recordedWorkTimeService').conflicts(db,[{type:parent.type,id:parent.id,technicianId:parent.technicianId,startAt:new Date(w.startedAt),endAt:new Date(w.endedAt)}]);if(conflicts.size)return refuse('TIME_OVERLAP','O técnico tem trabalho simultâneo noutro serviço. Reveja os horários.');}
  }
  let affectedShares=[];
  ...
  const value={schema:1,basis:rules.basis,reminderId,selection:body,contextHash:c.contextHash,previousHash:c.previousHash,origin,association,parent,peers,movements,comparison,affectedShares,...(affectedCosts?{affectedCosts}:{}),original};return rules.preview({available:true,...value,hash:writes.hash(value)},writes.hash);
}
...
async function command(actor,value,body){const owner=admin(actor),rid=id(value);parse(body,true);const {requestId,...payload}=body,request=writes.context(actor,rules.scope,rid,requestId,payload);return prisma.$transaction(async db=>{
  ...
  const createdAt=new Date(),event={schema:1,basis:rules.basis,id:requestId,owner,reminderId:rid,reason:body.reason,createdAt:createdAt.toISOString(),preview:p};
