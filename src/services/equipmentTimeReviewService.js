...
async function context(db,completionId,lock=false){
  ...
  const technical=originTrusted&&time.effective(rows.find(v=>v.id===completionId),prepared).valid,historical=technical?null:await require('./equipmentHistoricalResourceSource').selection(db,row,state);
  const editable=(technical||!!historical?.current||!!historical?.withdraw)&&state?.valid&&target?.valid&&rules.origin(origin)&&target.snapshot.originVisitType===p.visitType&&target.snapshot.originVisitId===p.visitId&&target.clientId===visit.clientId&&target.snapshot.poolId===visit.poolId&&visit.startAt instanceof Date&&+row.completedAt>=+visit.startAt;
  return {available:true,completionId,title:row.result?.plan?.title||'Revisão #'+completionId,origin,clientName:target?.clientName||'',poolName:pool?.name||'',technicianName:technician?.name||'',original,originalHash:r.hash(original),current,history:state?.history||[],journalValid:!!state?.valid,editable:!!editable,...(historical?.context?.legacy?{historicalReviewAvailable:true}:{}),...(historical?.current||historical?.withdraw?{historicalOrigin:{current:historical.current,withdrawalOnly:!historical.current,previous:historical.withdraw}}:{}),message:editable?historical?'Tempos históricos: declaração administrativa ligada à origem e evidência revistas. O recibo técnico original continua indisponível.':'A correção ficará atribuída à administração. O comprovativo original e os custos existentes serão conservados.':'Confirme a execução, a decisão comercial, a origem e o recibo original antes de corrigir tempos.',row,rows,visit,target,state,prepared,historical};
}
function publicContext(c){if(!c.available)return c;const{row,rows,visit,target,state,prepared,historical,...v}=c;return v;}
async function calculate(db,completionId,body,lock=false){
  ...
  if(c.historical&&!originReview)return refuse('HISTORICAL_ORIGIN_REVIEW','Confirme a origem histórica atual antes de declarar tempos. A declaração anterior pode ser anulada.');
  const proposed={action:body.action,record:body.workTime?time.create(time.parse(body.workTime),c.visit,c.origin.visitType,originReview?.hash):null},previous={headHash:c.state.headHash,action:c.state.action,record:c.state.record};
  ...
  const sourceHash=r.hash(json({...(c.historical?{historicalSource:c.historical.context.sourceHash,historicalState:c.historical.context.state?.headHash,historicalValid:c.historical.context.journalValid,historicalCurrent:c.historical.current?.hash||null}:{}),visit:c.visit,originals:c.rows.map(row=>journal.original(row,[...c.prepared.proofs.values()])),revisions:[...c.prepared.revisions].map(([id,s])=>({id,valid:s.valid,headHash:s.headHash})),associated:c.prepared.associated.get(c.origin.visitType+':'+c.origin.visitId),conflicts:[...c.prepared.conflicts].sort()}));
