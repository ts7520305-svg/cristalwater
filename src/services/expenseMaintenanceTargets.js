...
async function read(db, type, ids) {
  ...
  return rows.map(row=>{
    ...
    const valid=!!client && approved && !source.sourceIssue(row,s);
    ...
    return {type,id:row.id,clientId:facts.clientId,clientName:client?.name||null,label,valid,hash:r.hash(facts),snapshot:{...facts,label,clientName:client?.name||null},warning:valid?'Confirme a parcela e o mês desta despesa. O preço comercial e os custos da visita são registos separados.':'Reveja a execução e a decisão histórica da manutenção antes de atribuir despesas.'};
  });
}
