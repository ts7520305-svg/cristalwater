...
function context(actor, scope, resourceId, requestId, payload) {
  if (typeof requestId !== 'string' || !uuid.test(requestId) || !Number.isSafeInteger(resourceId) || resourceId <= 0 || !['VISIT_PHOTO', 'VISIT_COMPLETION', 'TECHNICIAN_ALERT', 'FIELD_STOCK_REQUEST', 'FIELD_PROBLEM_REPORT', 'FIELD_CLIENT_INTAKE', 'FIELD_CLIENT_APPROVAL', 'CLIENT_REPORT_SETTINGS', 'REGULAR_REPORT_ORIGIN', 'EXTRA_REPORT_ORIGIN', 'CLIENT_SERVICE_PLAN', 'CLIENT_SERVICE_GENERATION', 'EXTRA_VISIT_START', 'EXTRA_VISIT_PHOTO', 'EXTRA_VISIT_COMPLETION', 'EXTRA_VISIT_CORRECTION', 'EQUIPMENT_MAINTENANCE', 'EQUIPMENT_MATERIAL_REVIEW', 'EQUIPMENT_TIME_REVIEW', 'EQUIPMENT_HISTORY_REVIEW', 'REMINDER_RESOURCES', 'REMINDER_MATERIAL_CONSUMPTION', 'REMINDER_VISIT_ASSOCIATION', 'REMINDER_VISIT_RESOURCES', 'VISIT_INCOMPLETE', 'VISIT_RETURN', 'REPAIR_EXECUTION', 'REPAIR_WORK_INTERVAL', 'LABOR_COST_COMPOSITION', 'MONTHLY_EMAIL_REVIEW', 'MONTHLY_EMAIL_RETRY', 'COMPANY_CLOSURE', 'POOL_CALCULATION', 'ADMIN_ONBOARDING', 'SUPPLIER_CREATE', 'SUPPLIER_LINK_CREATE', 'TECHNICIAN_MANAGEMENT', 'FLEET_MANAGEMENT', 'VEHICLE_MANUAL_CONSUMPTION', 'TRANSPORT_GUIDE_CREATE', 'TRANSPORT_GUIDE_ITEMS', 'TRANSPORT_GUIDE_MANAGE', 'TRANSPORT_GUIDE_DOCUMENT', 'WORK_GUIDE_CLOSE', 'WORK_GUIDE_START', 'VEHICLE_MAINTENANCE_REVIEW', 'VEHICLE_ASSIGNMENT_REVIEW', 'VEHICLE_STOCK_PRESET_REVIEW', 'OPERATIONAL_RISK_RULES_REVIEW'].includes(scope)) fail('Conserve o pedido de campo original.');
