...
function context(actor, scope, resourceId, requestId, payload) {
  if (typeof requestId !== 'string' || !uuid.test(requestId) || !Number.isSafeInteger(resourceId) || resourceId <= 0 || !['VISIT_PHOTO', 'VISIT_COMPLETION', 'TECHNICIAN_ALERT', 'FIELD_STOCK_REQUEST', 'FIELD_PROBLEM_REPORT', 'FIELD_CLIENT_INTAKE', 'FIELD_CLIENT_APPROVAL', 'CLIENT_REPORT_SETTINGS', 'EXTRA_VISIT_START', 'EXTRA_VISIT_PHOTO', 'EXTRA_VISIT_COMPLETION', 'EXTRA_VISIT_CORRECTION', 'EQUIPMENT_MAINTENANCE', 'VISIT_INCOMPLETE', 'VISIT_RETURN'].includes(scope)) fail('Conserve o pedido de campo original.');
