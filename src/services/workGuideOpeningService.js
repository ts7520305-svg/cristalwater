...
async function start(actor,body,db=prisma){
 ...
 return db.$transaction(async tx=>{
  ...
  if(!guide){const preset=await tx.systemSetting.findUnique({where:{key:'vehicle_preset_'+vehicleId}});if(preset){const parsed=require('../../frontend/cw-vehicle-stock-preset-rules').parse(preset.value);if(!['VALID','EMPTY'].includes(parsed.state))fail('O modelo de materiais precisa de revisão.');items=parsed.items;source='VEHICLE_PRESET';}else{const last=await tx.transportGuide.findFirst({where:{vehicleId},orderBy:[{createdAt:'desc'},{id:'desc'}],include:{items:true}});if(last){items=last.items;inheritedFromId=last.id;source='LAST_GUIDE';}}}
