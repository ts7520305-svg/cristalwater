...
async function load(db,ids){
  ...
  const [reservations,movements]=await Promise.all([db.operationalLock.findMany({where:{id:{in:reservationIds}}}),db.stockMovement.findMany({where:{id:{in:movementIds}}})]);
...
async function prepare(db,repairId){
...
async function record(db,{repair,clientId,reservation,movements,actor,principal}){
  ...
  const snapshot={schema:1,basis,repairId:repair.id,clientId,poolId:repair.poolId,source:source(repair),completedAt,actor:{owner:actorOwner,label:String(actor)},reservation:reservationSnapshot(reservation),movements:movements.map(movementSnapshot)};
...
module.exports={eventType,basis,prepare,record,load,evaluate};
