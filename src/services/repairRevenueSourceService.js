...
async function load(db,invoices) {
  ...
  return {current:new Map(repairs.map(r=>[r.id,r])),proofs,counts};
}
function match(context,invoice,line) {
  ...
  // Billing and administrative closing are not proof of physical completion.
  return {reason:null,id:repair.id,poolId:s.poolId,label:s.repair.problem,status:normalize(repair.status),executionConfirmed:false};
}
