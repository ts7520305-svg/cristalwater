...
async function snapshot(monthRef) {
  ...
  try {
    return await prisma.$transaction(async db => {
      const [payments, monthly, invoices, clients] = await Promise.all([
      ...
      return { ...base, state:review ? 'REVIEW' : 'READY',
    ...
    return { ...base, state:'UNAVAILABLE', cash:null, monthDocuments:null, receivables:null, customerCredit:null, sourceUnavailable:true };
  }
}
