...
function inspectBackups({directory=path.resolve(__dirname,'../../backups'),now=new Date(),maxAgeHours=Number(process.env.BACKUP_MAX_AGE_HOURS||48)}={}){
 ...
 try{
  ...
  return{...base,ok:state==='RECENT_LOCAL_SQL',state,latest:{...latest,ageHours:Math.round(ageHours*100)/100}};
