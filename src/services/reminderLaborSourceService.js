...
function snapshot(row){
  ...
  if(!d?.workTime&&!d?.workIntervals)return null;
  return {version:d.workIntervals?2:1,basis:rules.basis,reminderId:row.reminderId,clientId:row.clientId,poolId:row.poolId,technicianId:row.technicianId,technicianName:'Técnico #'+row.technicianId,startedAt:row.startedAt,endedAt:row.endedAt,...(d.workIntervals?{workIntervals:d.workIntervals}:{}),durationSeconds:p.durationSeconds,sourceHash:p.targetHash,source:p.target,reason:e.reason,createdBy:e.owner,createdAt:e.createdAt,declaration:row.result};
}
