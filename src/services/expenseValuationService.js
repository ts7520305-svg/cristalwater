...
function selection(value, query = false) {
  ...
  return { kind: value.kind, targetType: value.targetType, targetId: id(value.targetId), purchaseItemId, quantity };
}
async function preview(db, expense, choice, lock = false) {
  ...
  const result = { version: 1, expenseId: expense.id, expenseVersion: expense.version, kind: choice.kind, targetType: choice.targetType, targetId: choice.targetId, targetHash: target.hash, clientId: target.clientId, monthRef: source.monthRef, purchaseItemId: choice.purchaseItemId, valuationKey: source.key, valuationHash: source.hash, quantity: calculation.quantity, quantityUnit: source.unit, amountCents, availableQuantity: data.decimal(available), label: source.label, calculation, source: source.snapshot };
...
async function apply(db, who, env, expense) {
  ...
  r.object(d, ['kind', 'targetType', 'targetId', 'targetHash', 'monthRef', 'purchaseItemId', 'quantity', 'amountCents', 'valuationHash', 'previewHash', 'reason', 'confirmed']);
