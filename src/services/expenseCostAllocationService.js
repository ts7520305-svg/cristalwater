...
function summary(expenses, monthRef, generatedAt) {
  ...
  // A damaged share journal may hide an attribution in another month. Without
  // a verifiable destination, no month can safely report a confirmed zero.
  const unplacedShareReviewCount = active.filter(e => e.maintenanceShareHistoryReview).length;
  const total = list => unplacedShareReviewCount || list.some(a => a.needsReview) ? null : sum(list.map(a => a.amountCents));
  const measured = valuation.summary(rows), valuations = unplacedShareReviewCount ? { ...measured, materialAmountCents: null, laborAmountCents: null } : measured;
  return { valuations, unplacedShareReviewCount, version: 1, monthRef, currency: 'EUR', generatedAt: generatedAt.toISOString(), state: reviewCount || unallocatedReviewCount || unplacedShareReviewCount ? 'REVIEW' : 'READY', coverage: 'REGISTERED_EXPENSE_ATTRIBUTION', completeOperatingCosts: false, profit: null,
