...
function validPreview(p) {
  try {
    ...
    return available === true && p.version === 1 && p.basis === basis && sha(hash) && r.hash(value) === hash && positive(p.expenseId) && positive(p.expenseVersion) && positive(p.allocationId) && p.allocationId === a.id && a.expenseId === p.expenseId && a.valuationType === 'LABOR' && ['REGULAR','EXTRA'].includes(a.targetType) && a.quantityUnit === 'SECOND' && a.voidedAt === null && duration(a) === p.parentDurationMs && r.hash(a) === p.allocationHash && p.monthRef === a.monthRef && positive(a.clientId) && positive(parentId(a)) && iso(a.targetSnapshot?.endAt) && a.targetSnapshot.endAt.slice(0, 7) === p.monthRef &&
      ...
      w.schema === 1 && w.basis === 'DECLARED_EQUIPMENT_WORK_INTERVAL' && iso(w.startAt) && iso(w.endAt) && w.durationMs === Date.parse(w.endAt) - Date.parse(w.startAt) && w.origin?.visitType === a.targetType && w.origin.visitId === parentId(a) && w.origin.clientId === a.clientId && w.origin.poolId === t.snapshot.poolId && positive(w.origin.technicianId) && w.origin.technicianId === a.valuationSnapshot?.source?.service?.technicianId && Date.parse(w.startAt) >= Date.parse(a.targetSnapshot.startAt) && Date.parse(w.endAt) <= Date.parse(a.targetSnapshot.endAt) && r.hash(w) === p.workTimeHash && calc && Object.entries(calc).every(([k, v]) => p[k] === v);
  } catch { return false; }
}
...
async function preview(db, expense, allocationId, completionId, lock = false, reminderMode = false) {
  ...
  const allocationBefore = allocationFacts(a), value = { version: 1, basis, expenseId: expense.id, expenseVersion: expense.version, allocationId, completionId, monthRef: a.monthRef, allocationBefore, allocationHash: r.hash(allocationBefore), parentDurationMs, used, workTime: w.record, workTimeHash: r.hash(w.record), target, ...calc };
