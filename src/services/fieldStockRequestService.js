...
async function create(actor, body={}) {
  ...
  return prisma.$transaction(async tx=>{
    ...
    // In visit briefings, an unassigned reminder is visible to every technician
    // visiting that pool. Keep the requester's scope; ADMIN receives the notice.
    const reminder=await tx.operationalReminder.create({data:{sourceKey:request?`stock-request:${owner}:${request.requestId}`:null,title,description,dueDate:new Date(),clientId:visit?.clientId||null,poolId:visit?.poolId||null,assignedToTechnicianId:technicianId,metadata}});
