...
async function renderGuidePdf(title, blocks, generatedAt = new Date()) {
  ...
  try {
    ...
    for (let page = pages.start; page < pages.start + pages.count; page++) {
      ...
      doc.font(fonts.regular).fontSize(8).fillColor('#596777').text(`Gerado em ${generatedAt.toLocaleString('pt-PT', { timeZone: 'Europe/Lisbon' })} (Europe/Lisbon)`, 42, 79, { lineBreak: false });
...
async function writePdfResponse(res, filename, title, draw) {
  const blocks = [];
  draw(blocks);
  const buffer = await renderGuidePdf(title, blocks);
...
module.exports = { renderGuidePdf, writePdfResponse, line, section, paragraph, tableRows };
