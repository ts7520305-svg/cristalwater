...
function recipientUserId(user={}){
 const id=Number(user.userId||user.id);
 return user.principalType==='USER'&&Number.isSafeInteger(id)&&id>0&&id<=2147483647?id:null;
}
...
function activeFor(user={}){
 ...
 return {...active,role:{in:['TECHNICIAN','TECH','TECNICO','TEAM_LEADER']},AND:[
  ...
  {OR:[{userId:null},{userId:0},...(recipientUserId(user)?[{userId:recipientUserId(user)}]:[])]},
...
module.exports={activeFor,canSeeFinancialNotification,recipientUserId};
