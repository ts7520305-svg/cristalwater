...
function confirmedDecision(proof,kind,id) {
  const s=proof?.source,r=proof?.result;
  if(!s||!r||r.ok!==true||r.schema!==1||s.kind!==kind||r.kind!==kind||s.sourceId!==id||r.sourceId!==id||s.reviewable!==true||s.reviewIssue!==null||!positive(s.clientId)||!positive(s.poolId)||r.clientId!==s.clientId||r.poolId!==s.poolId||!/^v1:[a-f0-9]{64}$/.test(r.expectedVersion)||s.expectedVersion!==r.expectedVersion||!Number.isFinite(Date.parse(r.reviewedAt))||typeof r.note!=='string'||typeof s.title!=='string'||typeof s.details!=='string')return false;
  if(proof.fingerprint!==hash({expectedVersion:r.expectedVersion,expectedClientId:r.clientId,expectedPoolId:r.poolId,mode:r.mode,amountCents:r.amountCents,note:r.note}))return false;
  return true;
}
function sourceIssue(row,s) {
  const kind=s.kind;
  if(!row)return 'MISSING_MAINTENANCE_SOURCE';
  if(!sameDate(s.completedAt,row.completedAt))return 'MAINTENANCE_SOURCE_CHANGED';
  if(kind==='EQUIPMENT'){
    const type=row.extraVisitId?'EXTRA':'REGULAR',visit=row.extraVisitId?row.extraVisit:row.visit;
    if(row.result?.ok!==true||row.result?.applied===false||!positive(row.version)||row.result?.plan?.version!==row.version+1||row.result?.plan?.poolId!==s.poolId||!sameDate(row.result?.completedAt,row.completedAt))return 'MAINTENANCE_SOURCE_CHANGED';
    if(!!row.visitId===!!row.extraVisitId||!visit||s.visitType!==type||s.visitId!==visit.id||s.poolId!==row.plan?.poolId||visit.poolId!==s.poolId||row.result?.plan?.id!==row.planId||row.result?.plan?.title!==s.title||row.notes!==s.details)return 'MAINTENANCE_SOURCE_CHANGED';
    if(visit.clientId!==s.clientId)return 'CLIENT_MISMATCH';
    // An intervention may finish while its parent visit is still in progress.
    if(!isCompletedVisitStatus(visit.status)&&normalize(visit.status)!=='IN_PROGRESS')return 'MAINTENANCE_SOURCE_CHANGED';
  }else{
    if(s.visitId!==null||s.visitType!==null||row.poolId!==s.poolId||row.title!==s.title||(row.description||'')!==s.details||!SERVICE_CATEGORIES.includes(row.category)||!isCompletedVisitStatus(row.status))return 'MAINTENANCE_SOURCE_CHANGED';
    if(row.clientId!==s.clientId)return 'CLIENT_MISMATCH';
  }
  return null;
}
function match(context, invoice, line) {
  ...
  if(!confirmedDecision(proof,kind,id))return {reason:'MAINTENANCE_DECISION_UNCONFIRMED'};
  ...
  const row=context.current.get(ref),issue=sourceIssue(row,s);if(issue)return {reason:issue};
...
module.exports={aliases,references,load,match,confirmedDecision,sourceIssue,visitSelect,key};
