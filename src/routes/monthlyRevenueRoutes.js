'use strict';
const router=require('express').Router(),service=require('../services/monthlyRevenueService'),r=require('../services/expenseLedgerRules');
...
router.use((error,req,res,next)=>{if(res.headersSent)return next(error);const status=[400,401,403,404,409,503].includes(error.status)?error.status:503;res.status(status).json({ok:false,error:error.status?error.message:'Não foi possível confirmar a repartição. Consulte o resultado do pedido antes de repetir.'});});
