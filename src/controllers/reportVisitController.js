...
async function generateVisitReport(req, res) {
  ...
  try {
    ...
    res.type("application/pdf").set("Content-Disposition", `inline; filename="relatorio-visita-${report.visitType === 'EXTRA' ? 'extra-' : ''}${report.visit.id}-${report.view}${report.historicalReview ? "-historical-review" : report.historicalOrigin ? "-historical-confirmed" : ""}.pdf"`).send(bytes);
  } catch (error) {
