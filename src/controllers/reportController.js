...
async function getMonthlyPrintableReport(req, res) {
  ...
  try {
    const report = await monthly.read(monthRef, onlyRequiresInvoice);
    const html = monthly.html(report);
    res.set({ 'X-CW-Report-Type':'monthly-print', 'X-CW-Report-Version':'2', 'X-CW-Month-Ref':monthRef, 'X-CW-Invoice-Filter':String(onlyRequiresInvoice) });
