...
async function confirm(user,id,body={}){
  ...
  return prisma.$transaction(async tx=>{
    ...
    for(const visitId of [...new Set([initial.row.reportedVisitId,initial.row.visitId])].sort((a,b)=>a-b))await require('../../services/incompleteVisitLifecycle').lock(tx,initial.row.visitType,visitId);
    ...
    if(row.visitType!==initial.row.visitType||row.visitId!==initial.row.visitId)fail(409,'A visita foi reatribuída. Atualize a lista.');
    ...
    const metadata={fingerprint,shortageId,visitType:row.visitType,visitId:row.visitId,movementId,vehicleId:tech.vehicleId,productName:row.productName,unit:row.unit,quantity,receivedBy:technicianId,receivedByName:tech.name,receivedAt:new Date().toISOString(),transferredBy:movement.createdBy};
