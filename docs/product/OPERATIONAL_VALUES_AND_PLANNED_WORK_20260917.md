...
Os endereços `/technician-profit-dashboard`, `/technician-profit` e `/alerts-financial` usam a mesma interface, mês UTC atual, fontes visíveis, valores desconhecidos explícitos, custos estimados identificados e detalhes documentais. Não dependem de Chart.js. Aceitam apenas respostas completas/coerentes, sem identidades/evidências duplicadas; mudança de período/conta, falha ou prazo de 20 segundos limpa os resultados antigos e permite recuperação. Nomes e produtos usam texto literal; rascunhos não relacionados são conservados.

A revisão final dos consumidores identificou `/alerts-financial`, que convertia margem ausente em zero e declarava ausência de alertas. Foi corrigido para mostrar margem por apurar e as fontes disponíveis, sem confirmar lucro/prejuízo. O teste das interfaces inclui agora as três páginas. Esta correção é posterior à primeira publicação `29a63fbc7ae05427cae048cb189bf4f00278775b`; o CI desse commit, por si só, não comprova a árvore final.
...
- `run-1789669696446`: três interfaces, incluindo alertas financeiros, 8173 ms. Ausência de falso «sem alertas», fontes/período/sessão/rede e 320/390/1440 aprovados. Capturas `reports/field-visual/operational-values-1789669701239`; 320 e 1440 inspecionadas.
