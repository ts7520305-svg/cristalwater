...
Atualizada após implementação da TASK188 em 16/09/2026. Substitui a fotografia inicial da TASK82 para decidir o próximo trabalho, preservando esse diagnóstico em `COMPLETENESS_20260915.md`. Esta matriz cruza os relatórios posteriores e as rotas atualmente revistas; não constitui auditoria exaustiva de todas as páginas/APIs nem conclusão a 100%.
...
| Edição de clientes | Correção preparada para preservar campos omitidos e guardar a auditoria atomicamente nos dois aliases; credenciais excluídas das respostas e da auditoria | TASK188; `CLIENT_EDIT_PRESERVATION_20260916.md`. Perda de dados reproduzida em QA; novo teste aprovado no CI `35075302355`, mas validação global/restauro aguardam a correção de sincronização do ensaio E2E. Versão, conflitos e recuperação do formulário continuam pendentes. |
...
1. Confirmar o CI da TASK188, que corrige a perda de dados ao editar clientes. Retomar versão, conflito entre alterações, confirmação e recuperação dos formulários. A falha de histórico na edição de piscinas foi reproduzida e continua pendente, incluindo os aliases e a ficha técnica. A recuperação dos pedidos de visita e avisos de pagamento está concluída nas TASK186–187.
