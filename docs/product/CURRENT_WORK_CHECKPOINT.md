...
- Última entrega remota verificada: TASK145, commit `e996fedae6c2cd9f3c17ed52b53cab5d2d604205`; workflow `34996754806` aprovado, incluindo 323 testes unitários, 17 scripts de navegador, 50 grupos operacionais, PostgreSQL 16 e restauro de 99 tabelas/11 anexos.
...
1. TASK143 publicada e verificada com 48 grupos e restauro. TASK144 protege a aplicação automática de crédito e o saldo anterior no Finance OS; TASK145 unifica o consumo da faturação mensal antiga. Confirmar workflow final com 50 grupos. Não repetir estas correções. Prosseguir pelo registo de caixa dos excedentes Finance OS (ainda incremento de saldo sem movimento de adiantamento), pela adição manual de crédito em `billingController.addCredit`/`frontend/billing.js`, e pela harmonização dos preços da faturação mensal antiga com os planos por cliente. O ecrã antigo `billing.js` também espera campos e ações que a API atual não fornece; rever a navegação e o percurso completo antes de o considerar funcional.
