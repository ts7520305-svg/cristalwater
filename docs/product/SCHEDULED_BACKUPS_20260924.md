...
Esta entrega cobre apenas a base de dados local e a preparação do agendamento. Não inclui uploads, cópia externa, entrega de alertas a fornecedores, instalação no VPS ou restauro operacional real. O painel mantém essas coberturas como não verificadas. CI completo e restauro da alteração permanecem pendentes.
