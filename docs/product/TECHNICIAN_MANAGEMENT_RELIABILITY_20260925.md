...
Nove grupos distintos tiveram execuções locais aprovadas. Correção de evidência na TASK365: a aprovação local da simulação de dois anos ocorreu antes de uma alteração final do auxiliar de técnicos. O código publicado confundia `status: CONFIRMED` no corpo JSON com o estado HTTP e esta combinação falhou na retoma. Esse resultado local não valida o auxiliar final da TASK364. A TASK365 corrige a distinção de tipos, acrescenta três testes para respostas diretas/com `data`/com `body` e repete com sucesso a simulação completa. Os registos originais são conservados.
...
TASK362 confirmada em [260/260 grupos e restauro nativo](evidence/20260925_task362_ci.json), 17 etapas, 41 migrações e 42m54s. Restauro de 127 tabelas/47 ficheiros com linhas e hashes iguais. TASK363 confirmada em [262/262 grupos e restauro nativo](evidence/20260925_task363_ci.json), 17 etapas, 41 migrações e 127 tabelas/47 ficheiros iguais, em 40m50s.
