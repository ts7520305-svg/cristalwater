...
- A decisão comercial guardada em `maintenance-billing:KIND:ID` deve confirmar a mesma origem, cliente, piscina, versão e assinatura do pedido. Só uma decisão EXTRA positiva, com recibo válido, pode contribuir; uma decisão INCLUDED não cria receita.
...
- Coerência do comprovativo de execução: versão, sucesso, data e título alterados deixam a origem por rever. Quantidade, preço, linha comercial, assinatura, cliente direto ausente/divergente e parentela cancelada também são exercitados.
...
Publicação sem força na branch de trabalho, CI nativo PostgreSQL 16 e restauro ainda pendentes nesta revisão. Não declarar o conjunto aprovado com base apenas nos testes locais. Sem merge na principal, deploy ou contactos reais.
