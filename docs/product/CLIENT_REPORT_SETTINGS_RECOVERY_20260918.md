...
Atualização TASK250–251: abertura autenticada nas configurações/centro mensal, pré-visualização CLIENT, opções no PDF/HTML e paginação individual implementadas. O CI `35310515176` aprovou 143/144 grupos e isolou overflow no select; confirmação nativa dos rótulos corrigidos pendente. Ver `AUTHENTICATED_VISIT_REPORTS_20260918.md`. As observações de código no final deste relatório conservam o diagnóstico histórico anterior a essa alteração; os cálculos do imprimível mensal, abertura pelos alertas e incorporação autorizada das fotos permanecem por tratar.
