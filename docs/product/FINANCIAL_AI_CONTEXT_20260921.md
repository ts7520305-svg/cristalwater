...
Primeira publicação: `9b737a69d714f3b16a0c1dccf7e380bec045dcc9`, árvore `7e2346520a90a213cc90c41809348d244dc8fc13`, idêntica à local. [CI 35644738229](https://github.com/ts7520305-svg/cristalwater/actions/runs/35644738229), job `106482312938`, aprovou 164/165 grupos, incluindo os novos de API (3907 ms) e interface financeira (11284 ms), mas não é aprovação global. O teste antigo `test-field-client-documents-ui.js` recusou um botão de 44 px medido pelo Chromium como 43.99998474121094 px após scroll; largura/overflow restantes estavam corretos. A comparação passa a admitir apenas 0.001 px de ruído numérico, mantendo o alvo de 44 px e todas as restantes asserções. O código do portal não muda. Restauro foi omitido devido à falha do grupo e precisa de confirmação na nova execução completa.
...
18 ficheiros: workflow, `frontend/admin-ai.html`, `frontend/admin-ai.js`, `frontend/crystal-os-v2-nav.js`, `frontend/sw.js`, fixture QA `financial-ai-server.js`, grupos `test-field-financial-ai.js`/`test-field-financial-ai-ui.js`, precisão da medição em `test-field-client-documents-ui.js`, runner, controller/auth/routes `aiAdmin`, serviços `aiAdminContextService`/`aiAdminLlmService`/`aiFinancialContextService`, este documento e `CURRENT_WORK_CHECKPOINT.md`.

Próxima evolução financeira: registo e conciliação de despesas/custos, com fontes históricas e atribuição por cliente/instalação, antes de calcular margem ou previsão. Já existem `StockPurchase`/`StockPurchaseItem` com custos de compra e `VehicleMaintenanceRecord.cost`; confirmar origem, período, pagamento e cobertura antes de integrar, sem duplicar esses registos nem confundir compra com consumo/pagamento. Preços/frequências continuam caso a caso por cliente, época e instalação, com três ou mais visitas quando necessário. A faturação fiscal com IVA continua no programa externo. A reconciliação explícita e auditável dos emails mensais antigos/FAILED/UNKNOWN/PENDING permanece uma tarefa própria. Este lote não inclui merge na principal, instalação na VPS ou contactos reais.
