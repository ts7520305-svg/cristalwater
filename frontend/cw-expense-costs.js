(function () {
  ...
  window.CWExpenseCosts = { create(host) {
    ...
    const period = window.CWExpenseCostPeriod.create(host), maintenanceLabor = window.CWExpenseMaintenanceLabor.create(host), maintenanceMaterial = window.CWExpenseMaintenanceMaterial.create(host);
    ...
    function controls() {
      ...
      document.querySelectorAll('#allocationForm input,#allocationForm select,#allocationForm textarea').forEach(n => n.disabled = !c.canWrite); valuation.controls(); period.controls(); maintenanceLabor.controls(); maintenanceMaterial.controls();
    }
    function draft() {
      valuation.draft(); maintenanceLabor.draft(); maintenanceMaterial.draft();
    ...
    function clear() {
      draft(); valuation.clear(); period.clear(); maintenanceLabor.clear(); maintenanceMaterial.clear(); revision++; pickerRevision++; chosen = client = null; editingId = null;
    ...
    function render(expense) {
      ...
      maintenanceLabor.clear(); maintenanceMaterial.clear(); period.clear(); editingId = expense.id; chosen = client = null; pickerRevision++; pickerPage = 1; pickerTotal = 0;
      ...
      for (const a of expense.allocations) {
        ...
        maintenanceLabor.render(row, a); maintenanceMaterial.render(row, a);
        ...
        if (a.targetType === 'MAINTENANCE_REMINDER' && a.valuationType === 'LABOR') { const w=a.valuationSnapshot?.source?.workInterval,t=w?.snapshot; if(!positive(w?.id)||t?.reminderId!==a.serviceReminderId||t?.clientId!==a.clientId||!positive(t?.technicianId))throw Error('Declaração de trabalho do lembrete incompleta.');node(row,'p','Declaração #'+w.id+' · Técnico #'+t.technicianId+' · '+window.CWRepairLabor.formatTime(t.startedAt)+' a '+window.CWRepairLabor.formatTime(t.endedAt));const link=node(row,'a','Consultar recursos e histórico do lembrete');link.href='/reminder-resources?reminderId='+a.serviceReminderId; }
        if (a.targetType === 'MAINTENANCE_REMINDER' && a.valuationType === 'MATERIAL') { const link=node(row,'a','Consultar consumo e histórico dos materiais');link.href='/reminder-materials?reminderId='+a.serviceReminderId; }
        if (a.valuationType !== 'MANUAL') { node(row, 'p', (a.valuationType === 'MATERIAL' ? 'Consumo valorizado: ' : 'Tempo valorizado: ') + a.quantity + ' ' + (a.quantityUnit === 'SECOND' ? 'segundos' : a.quantityUnit)); if (a.needsReview && !a.reviewReasons?.includes('RECORDED_TIME_OVERLAP')) node(row, 'p', 'A origem mudou. Anule e calcule novamente para confirmar o novo custo.'); }
    ...
    function receipt(result, record) { valuation.receipt(result, record); period.receipt(result, record); maintenanceLabor.receipt(result, record); maintenanceMaterial.receipt(result, record); if (result.applied && ['ALLOCATE_COST','REVIEW_COST','VOID_COST','SET_LABOR_BASIS','VALUE_MATERIAL','VALUE_LABOR'].includes(record.envelope.command)) { try { sessionStorage.removeItem(key(record.envelope.expenseId)); } catch {} editingId = null; } }
    ...
    function observe() {
      valuation.observe(); period.observe(); maintenanceLabor.observe(); maintenanceMaterial.observe();
    ...
    async function verify(result, record) {
      await valuation.verify(result, record); await period.verify(result, record); await maintenanceLabor.verify(result, record); await maintenanceMaterial.verify(result, record); await maintenance.receipt(result,record,host.hash);
