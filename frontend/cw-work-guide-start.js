(function () {
  'use strict';
  const R = window.CWWorkGuideStartRules, copy = window.CWWorkGuideStartCopy;
  const $ = id => document.getElementById(id), panel = $('workStarter'), jobs = {};
  ...
  function clearFields() { $('consumeForm').reset(); $('technician').replaceChildren(); $('confirm').checked = false; }
  function active() {
  ...
  async function request(path, job, config = {}) {
    const res = await fetch('/api/work-guide-start' + path, { ...config, headers: { Authorization: 'Bearer ' + actor.token, 'Content-Type': 'application/json' }, cache: 'no-store', redirect: 'error', signal: job.controller.signal });
    if (!current(job)) throw Error('stale');
    if ([401, 403].includes(res.status)) { invalid = true; active(); throw Error('session'); }
    const trusted = res.headers.get('x-cw-work-start') === 'work-guide-start-v1' && res.headers.get('x-cw-owner') === actor.owner && res.headers.get('cache-control') === 'private, no-store' && (res.headers.get('content-type') || '').startsWith('application/json');
    const value = await res.json();
    if (!current(job) || !trusted) throw Error('unconfirmed');
    if (res.status !== 200) throw Object.assign(Error('refused'), value?.ok === false && value.version === 1 && value.owner === actor.owner && typeof value.code === 'string' && value.code.startsWith('WORK_START_') ? { status: res.status, code: value.code } : {});
    if (!R.base(value, actor.owner)) throw Error('unconfirmed');
    return value;
  }
  function collect() {
    const selected=$('technician').value;if(!selected)return null;
    return R.input({vehicleId:target?.vehicle.id,technicianId:selected==='NONE'?null:R.id(selected),startKmMode:$('startKmMode').value,startKm:$('startKmMode').value==='NONE'?null:$('startKm').value.trim().replace(',','.'),notes:$('workNotes').value||null,reason:$('notes').value});
  }
  function choose(choice) {
    if (!active() || blocked() || busy() || pending || target || !data || choice.openWorkCount) return;
    target = choice; review = proposal = receipt = null; clearFields();
    $('technician').replaceChildren();
    if(actor.role==='ADMIN'){$('technician').append(new Option(t('selectTechnician'),''),new Option(t('unassigned'),'NONE'));}
    for(const tech of choice.technicians)$('technician').append(new Option(named(tech),String(tech.id)));
    writeState = 'editing'; render();$('writePanel').scrollIntoView({block:'start'}); $('technician').focus({preventScroll:true});
  }
  function facts(dl,c) {
    fact(dl,'plate',c.vehicle.plate);fact(dl,'vehicleKmBefore',c.vehicle.currentKm);
    if(c.source){fact(dl,'technician',named(c.technician));fact(dl,'source',t(c.source));fact(dl,'AT',c.transport?'#'+c.transport.id+' — '+(c.transport.codeAT||t('missing')):t('none'));}
    else fact(dl,'openWorks',c.openWorkCount);
  }
  function stock(box,items) { box.replaceChildren(); if(!items.length)box.append(node('p',t('noItems')));for(const item of items){const div=node('article','');div.className='hub-card';div.append(node('h4',item.name+' · '+item.unit));const dl=node('dl','');fact(dl,'materialType',item.type);fact(dl,'initial',item.quantity);div.append(dl);box.append(div);} }
  ...
  function render() {
    ...
    $('writeStatus').textContent = t(writeState); $('targetIdentity').textContent = target ? t('plate')+' · '+target.vehicle.plate : '';
    ...
    $('currentStock').replaceChildren();$('currentStock').hidden=true;
    ...
    $('modeHint').textContent=t($('startKmMode').value==='NONE'?'noneReading':'set');
    const chosen=target?.technicians.find(tech=>String(tech.id)===$('technician').value);
    $('technicianHint').textContent=chosen?named(chosen):$('technician').value==='NONE'?t('unassigned'):'';
    $('startKm').disabled ||= $('startKmMode').value==='NONE';
    $('reviewPanel').hidden=!review||blocked()||!!pending;$('reviewFacts').replaceChildren();stock($('reviewStock'),review?.choice.items||[]);
    if(review){const dl=$('reviewFacts');facts(dl,review.choice);fact(dl,'startKm',review.plan.startKm);fact(dl,'vehicleKmAfter',review.plan.vehicleKmAfter);fact(dl,'workNotes',review.proposal.notes);fact(dl,'notes',review.proposal.reason);fact(dl,'internalAlert',t(review.plan.missingTransportGuide?'yes':'no'));}
    ...
    if(receipt){const dl=$('receiptFacts');fact(dl,'plate','#'+receipt.vehicleId);fact(dl,'confirmedAt',date(receipt.receipt.confirmedAt));fact(dl,'audit','#'+receipt.auditId);if(receipt.work){fact(dl,'work','#'+receipt.work.id);fact(dl,'source',t(receipt.plan.source));fact(dl,'startKm',receipt.work.startKm);fact(dl,'vehicleKmAfter',receipt.plan.vehicleKmAfter);fact(dl,'workNotes',receipt.work.notes);fact(dl,'notes',receipt.proposal.reason);}}
    ...
    if (data) {
      ...
      for(const c of data.rows){const card=node('article',''),dl=node('dl',''),b=node('button',t('choose'));card.className='hub-card';card.dataset.vehicleId=c.vehicle.id;card.append(node('h3',c.vehicle.plate+' · #'+c.vehicle.id));facts(dl,c);card.append(dl);b.type='button';b.disabled=blocked()||busy()||!!pending||!!target||c.openWorkCount>0;b.onclick=()=>choose(c);card.append(b);if(c.openWorkCount){card.append(node('p',t('alreadyOpen')));if(c.existingWorkId){const link=node('a',t('openExisting'));link.href='/work-guide-close?workGuideId='+c.existingWorkId+'&lang='+language;card.append(link);}}$('guideList').append(card);}
  ...
  if (R.id(url.searchParams.get('vehicleId'))) { query.vehicleId = url.searchParams.get('vehicleId'); $('guideFilter').value = query.vehicleId; }
  try { actor = identity(); if (!actor) throw Error(); key = 'cw:work-guide-start:v1:' + actor.owner; document.body.dataset.requiredRole = actor.role; } catch (_) { invalid = true; }
  $('startKmMode').addEventListener('change',()=>{if($('startKmMode').value==='NONE'){$('startKm').value='';}render();});
  $('technician').addEventListener('change',render);
  ...
  $('filters').addEventListener('submit', e => { e.preventDefault(); const filter = { q: $('search').value.trim(), ...($('guideFilter').value.trim() ? { vehicleId: $('guideFilter').value.trim() } : {}) }; if (!R.query(filter)) { abort('list'); data = null; state = 'error'; render(); return; } query = filter; page = 1; load(); });
