(function () {
  ...
  const maintenanceIds = ['maintenanceCompletionId','serviceReminderId'];
  // Receipts issued before the additive migration retain their original shape/hash.
  const receiptFields = a => ['REGULAR','EXTRA','REPAIR'].includes(a?.targetType) && maintenanceIds.every(k => !Object.hasOwn(a,k)) ? fields.filter(k => !maintenanceIds.includes(k)) : fields;
  ...
  const targetId = a => a.targetType === 'REGULAR' ? a.visitId : a.targetType === 'EXTRA' ? a.extraVisitId : a.targetType === 'REPAIR' ? a.repairId : window.CWExpenseMaintenance.targetId(a);
  ...
  window.CWExpenseCostPeriod = { create(host) {
    ...
    async function verifyPreview(p, expenseId, version, allocation) {
      ...
      if (!p || available !== true || p.version !== 1 || p.basis !== 'CONFIRMED_SERVICE_EXECUTION_MONTH_UTC' || p.expenseId !== expenseId || p.expenseVersion !== version || !positive(p.allocationId) || !positive(p.clientId) || !positive(p.targetId) || !positive(p.amountCents) || !month(p.fromMonth) || !month(p.toMonth) || p.fromMonth === p.toMonth || !a || Object.keys(a).sort().join() !== [...receiptFields(a)].sort().join() || a.id !== p.allocationId || a.expenseId !== expenseId || a.monthRef !== p.fromMonth || a.amountCents !== p.amountCents || a.clientId !== p.clientId || a.targetType !== p.targetType || !['REGULAR','EXTRA','REPAIR',...window.CWExpenseMaintenance.types].includes(a.targetType) || targetId(a) !== p.targetId || a.valuationType !== 'MANUAL' || a.voidedAt !== null || a.voidReason !== null || !iso(a.createdAt) || !positive(a.createdById) || !(a.reviewedAt === null || iso(a.reviewedAt)) || ['valuationKey','valuationHash','valuationSnapshot','quantity','quantityUnit','purchaseItemId','activeMeasurementKey'].some(k => a[k] !== null) || ['visitId','extraVisitId','repairId','maintenanceCompletionId','serviceReminderId'].some(k => (a[k] ?? null) !== (k === ({REGULAR:'visitId',EXTRA:'extraVisitId',REPAIR:'repairId',MAINTENANCE_EQUIPMENT:'maintenanceCompletionId',MAINTENANCE_REMINDER:'serviceReminderId'}[a.targetType]) ? p.targetId : null))) throw Error('Correção do período não confirmada.');
      const s = a.targetSnapshot, facts = ['type','id','clientId','poolId','status','startAt','endAt', ...(a.targetType === 'REPAIR' ? ['executionBasis','executionProofId','executionFingerprint','materialMode'] : window.CWExpenseMaintenance.types.includes(a.targetType) ? window.CWExpenseMaintenance.fields : [])];
      ...
      if(window.CWExpenseMaintenance.types.includes(a.targetType)) window.CWExpenseMaintenance.facts(window.CWExpenseMaintenance.allocation(a));
