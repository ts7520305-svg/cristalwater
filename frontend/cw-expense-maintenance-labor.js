(function () {
  ...
  window.CWExpenseMaintenanceLabor = { create(host) {
    ...
    let revision = 0, stamp = '', selected = null, preview = null, voiding = null, page = 1, total = 0;
    ...
    const draftKey = () => preview ? key(preview.expenseId, preview.allocationId, preview.completionId, 'share') : voiding ? key(voiding.share.expenseId, voiding.share.allocationId, voiding.share.id, 'void') : null;
    ...
    function picker(visible) { el('maintenanceLaborCandidates').hidden = !visible; el('maintenanceLaborPrevious').hidden = !visible; el('maintenanceLaborNext').hidden = !visible; el('maintenanceLaborChange').hidden = visible || !!voiding; }
    function clear() { picker(true); revision++; selected = preview = voiding = null; stamp = ''; page = 1; total = 0; el('maintenanceLaborBox').hidden = true; el('maintenanceLaborCandidates').replaceChildren(); el('maintenanceLaborFacts').replaceChildren(); el('maintenanceLaborStatus').textContent = ''; el('maintenanceLaborForm').reset(); }
    function controls() {
      const c = context();
    ...
    function showFacts(p, removing = false) {
      ...
      node(facts, 'p', 'Tempo desta revisão: ' + (p.workTime.durationMs / 1000) + ' s · Tempo valorizado da visita: ' + (p.parentDurationMs / 1000) + ' s · ' + p.monthRef + ' (UTC)');
    ...
    async function choose(id) {
      ...
      try {
        const result = await host.request('/' + c.expense.id + '/maintenance-labor-preview?allocationId=' + allocation.id + '&completionId=' + id);
    ...
    async function candidates() {
      ...
      try {
        const result = await host.request('/' + c.expense.id + '/maintenance-labor-candidates?allocationId=' + selected.id + '&page=' + page), data = result.candidates;
        if (!active() || rev !== revision || stamp !== context().stamp) return;
        ...
        total = data.total; el('maintenanceLaborStatus').textContent = data.available ? total + ' revisões · Página ' + page : data.message;
        for (const row of data.rows) {
          if (!positive(row.id) || typeof row.label !== 'string' || !['MISSING','RECORDED','REVIEW'].includes(row.workTimeState) || typeof row.targetConfirmed !== 'boolean' || typeof row.shared !== 'boolean') throw Error('Revisão recebida incompleta.');
          const card = node(el('maintenanceLaborCandidates'), 'article', '', 'row'); node(card, 'strong', row.label);
          node(card, 'p', row.shared ? 'Parcela já atribuída neste documento.' : row.workTimeState === 'MISSING' ? 'Falta registar o tempo próprio.' : row.workTimeState === 'REVIEW' ? 'Tempo próprio por rever.' : !row.targetConfirmed ? 'Execução ou decisão comercial por confirmar.' : (row.durationMs / 1000) + ' s de trabalho registados.');
    ...
    el('maintenanceLaborForm').addEventListener('submit', event => {
      ...
      if (preview) void host.execute(commands[0], { allocationId: preview.allocationId, completionId: preview.completionId, amountCents: preview.amountCents, previewHash: preview.hash, reason, confirmed: true });
