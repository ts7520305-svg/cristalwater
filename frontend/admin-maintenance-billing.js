(() => {
  ...
  function renderRows() {
    ...
    for (const row of rows) {
      ...
      if (row.kind === 'EQUIPMENT') { const historyLink=node('a','Rever origem histórica',card);historyLink.href='/equipment-history-review.html?completionId='+row.sourceId+'&poolId='+row.poolId; const timeLink=node('a','Consultar / corrigir tempos',card);timeLink.href='/equipment-time-review.html?completionId='+row.sourceId+'&poolId='+row.poolId;const link = node('a', 'Consultar / corrigir materiais', card); link.href = '/equipment-material-review?completionId=' + row.sourceId + '&poolId=' + row.poolId; }
