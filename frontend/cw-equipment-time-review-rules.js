(function(root,factory){
  ...
  function record(v){
    ...
    const rows=intervals(v),schema=v.schema===3?Array.isArray(v.intervals)?2:1:v.schema,data=schema===2?{intervals:rows.map(w=>({startAt:w?.startAt,endAt:w?.endAt}))}:{startAt:v.startAt,endAt:v.endAt};input(data);
    if(v.schema===3&&!sha(v.originReviewHash))fail();
    if(schema===1){if(!fields(v,['schema','basis','startAt','endAt','durationMs','origin',...(v.schema===3?['originReviewHash']:[])])||v.basis!=='DECLARED_EQUIPMENT_WORK_INTERVAL')fail();}
    else if(schema!==2||!fields(v,['schema','basis','intervals','durationMs','origin',...(v.schema===3?['originReviewHash']:[])])||v.basis!=='DECLARED_EQUIPMENT_WORK_INTERVALS'||!Array.isArray(v.intervals)||rows.some(w=>!fields(w,['startAt','endAt','durationMs'])||w.durationMs!==Date.parse(w.endAt)-Date.parse(w.startAt)))fail();
  ...
  async function preview(p,hash){
    ...
    if(p.schema===2){
      const history=typeof module==='object'&&module.exports?require('./cw-equipment-history-rules'):globalThis.CWEquipmentHistoryRules,proof=await history.revision(p.originReview,hash),source=proof.revision.preview;
      if(source.completionId!==p.completionId||source.baseHash!==p.baseHash||source.proposed.action!=='REPLACE'||p.proposed.action==='REPLACE'&&(await hash(source.origin)!==await hash(p.origin)||source.targetHash!==p.targetHash||p.proposed.record?.schema!==3||p.proposed.record.originReviewHash!==proof.hash)||p.proposed.action==='WITHDRAW'&&(p.previous.record?.schema!==3||p.previous.record.originReviewHash!==proof.hash))fail();
    }else if(p.previous.record?.schema===3||p.proposed.record?.schema===3)fail();
  ...
  async function share(p,hash){
    const e=(await revision(p.workTimeRevision,hash)).revision;if(e.completionId!==p.completionId||e.preview.proposed.action!=='REPLACE'||await hash(e.preview.proposed.record)!==await hash(p.workTime)||p.workTime?.schema===3&&(e.preview.schema!==2||e.preview.targetHash!==p.target?.hash))fail();return p;
  }
