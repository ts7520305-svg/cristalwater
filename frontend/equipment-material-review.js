(function () {
  ...
  function drawRecord(parent, record) { if(!historyMode&&!timeMode&&record?.schema===2)node(parent,'p','Declaração administrativa de materiais históricos, ligada à evidência de origem.'); node(parent, 'pre', materialText(record)); }
  ...
  async function load() {
    ...
    try {
      ...
      if(historyMode&&value.current.state==='ATTESTED'){const link=node(el('eligibility'),'a',' Declarar / rever materiais históricos');link.href='/equipment-material-review?completionId='+completionId+'&poolId='+o.poolId;}
