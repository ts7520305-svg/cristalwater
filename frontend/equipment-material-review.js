(function () {
  'use strict';
  const el = id => document.getElementById(id), proof = window.CWEquipmentMaterialReviewRules;
  ...
  const states = { MISSING:'Materiais não registados',NONE:'Sem materiais, confirmado',DECLARED:'Aguarda consumos e fecho da visita',MATCHED:'Compatível com o consumo líquido da visita',REVIEW:'Materiais por rever',WITHDRAWN:'Declaração anulada; materiais por confirmar' };
  ...
  let draft = { action: 'DECLARED', items: [{ productName:'',unit:'',quantity:'' }], reason:'' };
  ...
  function controls() {
    ...
    if (pending && !invalid) el('pendingText').textContent = 'Revisão #' + pending.completionId + '\n' + (pending.body.action === 'WITHDRAW' ? 'Anular declaração' : materialText(pending.body.materials)) + '\nMotivo: ' + pending.body.reason;
  }
  function materialText(record) { return !record ? 'Materiais por confirmar.' : record.mode === 'NONE' ? 'Sem materiais, por declaração explícita.' : record.items.map(i => i.productName + ' · ' + i.quantity + ' ' + i.unit).join('\n'); }
  ...
  const draftKey = () => 'cw-equipment-material-draft:' + principal.owner + ':' + completionId;
  function validDraft(value) { return value && ['DECLARED','NONE','WITHDRAW'].includes(value.action) && Array.isArray(value.items) && value.items.length <= 20 && typeof value.reason === 'string' && value.reason.length <= 500 && value.items.every(i => proof.fields(i, ['productName','unit','quantity']) && [['productName',160],['unit',24],['quantity',30]].every(([k,max]) => typeof i[k] === 'string' && i[k].length <= max)); }
  ...
  function drawItems() {
    ...
    for (const [index, item] of draft.items.entries()) {
      ...
      for (const [key, title, max] of [['productName','Produto',160],['unit','Unidade',24],['quantity','Quantidade',30]]) {
        const label = node(fields, 'label', title), input = node(label, 'input', ''); input.value = item[key]; input.maxLength = max; input.dataset.material = key; input.dataset.index = index; if (key === 'quantity') input.inputMode = 'decimal';
        input.oninput = () => { if (!active() || busy || pending) return; item[key] = input.value; clearPreview(); saveDraft(); };
      }
      const remove = node(card, 'button', 'Remover produto'); remove.type = 'button'; remove.className = 'secondary';
  ...
  async function load() {
    ...
    try {
      ...
      const value = (await request('/completions/' + completionId + '/materials')).declaration;
      ...
      if (!draftHydrated) { const record = value.current.record; if (record && ['NONE','DECLARED'].includes(record.mode) && Array.isArray(record.items)) draft = { action:record.mode,items:record.items.map(i => ({ ...i })),reason:'' }; draftHydrated = true; el('action').value = draft.action; drawItems(); }
  ...
  async function calculate() {
    ...
    try {
      const body = payload(), p = (await request('/completions/' + completionId + '/material-preview', body)).preview;
      if (!active() || ticket !== editEpoch || read !== readEpoch) return;
      check(p?.available, p?.message); await proof.preview(p, hash);
      check(p.completionId === completionId && p.baseHash === detail.originalHash && equal(p.origin, detail.origin) && p.previous.headHash === (detail.history.at(-1)?.hash || null) && equal(p.previous.record, detail.current.record) && p.proposed.action === body.action && equal(p.proposed.record ? { mode:p.proposed.record.mode,items:p.proposed.record.items } : null, body.materials));
      ...
      if (!p.affectedShares.length) node(box, 'p', 'Não há parcelas ativas de materiais desta visita.');
      for (const s of p.affectedShares) { const line = node(box, 'p', 'Revisão #' + s.completionId + ' · ' + s.quantity + ' unidades · Parcela original ' + money(s.amountCents) + ' · '); const link = node(line, 'a', 'Abrir despesa #' + s.expenseId); link.href = '/admin-expenses?expenseId=' + s.expenseId + '&allocationId=' + s.allocationId; }
      node(box, 'p', 'As parcelas existentes conservam os valores e as reservas até à sua revisão ou anulação em Despesas. Esta alteração não movimenta stock nem altera os registos de trabalho, compras ou pagamentos.');
  ...
  async function validPending(row) {
    ...
    check(proof.fields(row.body, ['requestId','action','materials','previewHash','reason','confirmed']) && row.body.confirmed === true && typeof row.body.reason === 'string' && row.body.reason.trim() === row.body.reason && row.body.reason.length >= 3 && row.body.reason.length <= 500);
    await proof.preview(row.preview, hash); check(row.preview.completionId === row.completionId && row.preview.hash === row.body.previewHash && row.body.action === row.preview.proposed.action && equal(row.body.materials, row.preview.proposed.record ? { mode:row.preview.proposed.record.mode,items:row.preview.proposed.record.items } : null)); return row;
  }
  ...
  async function accept(result, row) {
    ...
    if (result.applied && row.completionId === completionId && draft.reason.trim() === row.body.reason) {
      let matched = false; try { matched = equal(payload(), { action:row.body.action,materials:row.body.materials }); } catch (_) {}
  ...
  async function execute(mode) {
    ...
    busy = true; controls();
    try { await navigator.locks.request('cw-equipment-material-review:' + principal.owner, { ifAvailable:true }, async lock => {
      ...
      const result = mode === 'check' ? await request('/material-review-requests/' + stored.body.requestId) : await request('/completions/' + stored.completionId + '/material-review', stored.body);
  ...
  (async () => { try {
    ...
    try {
      check(navigator.locks && crypto.subtle && crypto.randomUUID); db = await new Promise((resolve, reject) => { const q = indexedDB.open('cw-equipment-material-review-v1', 1); q.onupgradeneeded = () => q.result.createObjectStore('state'); q.onsuccess = () => resolve(q.result); q.onerror = q.onblocked = () => reject(Error('Storage')); }); await syncPending();
