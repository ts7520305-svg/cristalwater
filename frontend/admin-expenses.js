(function () {
  ...
  const commandNames = { CREATE: 'Nova despesa', EDIT: 'Correção da despesa', CANCEL: 'Anulação da despesa', REOPEN: 'Reabertura', RECORD_PAYMENT: 'Registo de pagamento', REVERSE_PAYMENT: 'Correção de pagamento', ADD_EVIDENCE: 'Novo comprovativo', VOID_EVIDENCE: 'Anulação de comprovativo', ALLOCATE_COST: 'Atribuição de custo', REVIEW_COST: 'Revisão de atribuição', VOID_COST: 'Anulação de atribuição', CORRECT_COST_PERIOD: 'Correção do mês da atribuição', SET_LABOR_BASIS: 'Base de custo do trabalho', VALUE_MATERIAL: 'Valorização de consumo', VALUE_LABOR: 'Valorização do tempo' };
  Object.assign(commandNames, {SHARE_MAINTENANCE_MATERIAL:'Repartição de materiais com manutenção',VOID_MAINTENANCE_MATERIAL_SHARE:'Anulação da parcela de materiais',SHARE_MAINTENANCE_LABOR:'Repartição do trabalho com manutenção',VOID_MAINTENANCE_LABOR_SHARE:'Anulação da parcela da manutenção',SET_LABOR_DISTRIBUTION:'Repartição do trabalho por técnico/período',VOID_LABOR_DISTRIBUTION:'Anulação da repartição do trabalho'});
  ...
  function controls() {
    ...
    costsUI.controls();
    if (pending && !invalid) { const e = pending.envelope, d = e.data; el('pendingPreview').textContent = [commandNames[e.command] + (e.expenseId ? ' · Despesa #' + e.expenseId : ''), d.title, d.supplierName, d.documentNumber ? 'Documento: ' + d.documentNumber : '', d.amountCents ? 'Montante: ' + money(d.amountCents) : '', d.expenseDate ? 'Data do documento: ' + d.expenseDate : '', d.dueDate ? 'Vencimento: ' + d.dueDate : '', d.paidOn ? 'Pagamento efetuado em: ' + d.paidOn : '', d.name ? 'Comprovativo: ' + d.name : '', d.fromMonth && d.toMonth ? 'Correção de mês: ' + d.fromMonth + ' → ' + d.toMonth + ' (UTC) · Atribuição #' + d.allocationId : '', d.monthRef ? 'Mês da atribuição: ' + d.monthRef : '', d.targetType ? 'Destino: ' + ({COMPANY:'Empresa',CLIENT:'Cliente',REGULAR:'Visita regular',EXTRA:'Visita extra',REPAIR:'Reparação',MAINTENANCE_EQUIPMENT:'Manutenção de equipamento',MAINTENANCE_REMINDER:'Lembrete de serviço'}[d.targetType]) + (d.targetId ? ' #' + d.targetId : '') : '', d.workIntervalId ? 'Intervalo declarado #' + d.workIntervalId : '', d.quantity ? 'Quantidade: ' + d.quantity + (d.kind === 'LABOR' ? ' segundos' : '') : '', d.paidMinutes ? 'Tempo pago: ' + d.paidMinutes + ' minutos' : '', d.periodStart ? 'Período: ' + d.periodStart + ' a ' + d.periodEnd : '', d.technicianId ? 'Técnico #' + d.technicianId : '', d.reason ? 'Motivo: ' + d.reason : ''].filter(Boolean).join('\n'); }
    if(pending&&!invalid){const d=pending.envelope.data;if(d.completionId)el('pendingPreview').textContent+='\nManutenção #'+d.completionId+' · Atribuição de origem #'+d.allocationId;if(d.reminderId)el('pendingPreview').textContent+='\nLembrete #'+d.reminderId+' · Atribuição de origem #'+d.allocationId;if(d.shareId)el('pendingPreview').textContent+='\nParcela '+d.shareId+' · Atribuição de origem #'+d.allocationId;for(const [i,p] of (Array.isArray(d.parts)?d.parts:[]).entries())el('pendingPreview').textContent+='\nParcela '+(i+1)+' · Técnico #'+p.technicianId+' · '+p.periodStart+' a '+p.periodEnd+' · '+p.paidMinutes+' minutos · '+money(p.amountCents);if(d.laborPart)el('pendingPreview').textContent+='\nParcela de trabalho: '+d.laborPart;if(d.distributionId)el('pendingPreview').textContent+='\nRepartição #'+d.distributionId;}
  }
  ...
  async function openDetail(id) {
    ...
    try {
      ...
      for (const event of result.events) { const row = node(el('eventList'), 'div', '', 'row'); node(row, 'strong', commandNames[event.command] || event.command); node(row, 'p', event.actorName + ' · ' + new Date(event.createdAt).toLocaleString('pt-PT')); node(row, 'p', event.result.applied ? event.result.reason || 'Registo confirmado.' : event.result.message); if (event.command === 'CORRECT_COST_PERIOD' && event.result.applied) node(row, 'p', 'Mês ' + event.result.preview.fromMonth + ' → ' + event.result.preview.toMonth + ' · ' + money(event.result.preview.amountCents) + ' · Atribuição #' + event.result.allocationBefore.id + ' substituída pela #' + event.result.allocation.id); if (event.result.before && event.result.after) node(row, 'p', 'Total anterior: ' + money(event.result.before.amountCents) + ' · Total confirmado: ' + money(event.result.after.amountCents)); }
      costsUI.render(e);
  ...
  async function verify(result, record) {
    ...
    if (result.applied && ['ALLOCATE_COST','REVIEW_COST','VOID_COST'].includes(e.command)) { const a = result.allocation; if (!a || !positive(a.id) || a.expenseId !== e.expenseId || !positive(a.amountCents) || (e.command === 'ALLOCATE_COST' ? a.amountCents !== e.data.amountCents || a.monthRef !== e.data.monthRef || a.targetType !== e.data.targetType || (a.targetType === 'CLIENT' ? a.clientId : a.targetType === 'REGULAR' ? a.visitId : a.targetType === 'EXTRA' ? a.extraVisitId : a.targetType === 'REPAIR' ? a.repairId : window.CWExpenseMaintenance.types.includes(a.targetType) ? window.CWExpenseMaintenance.targetId(a) : null) !== e.data.targetId : a.id !== e.data.allocationId) || e.command !== 'VOID_COST' && a.targetHash !== e.data.targetHash || e.command === 'VOID_COST' && (!a.voidedAt || a.activeKey !== null)) throw Error('Atribuição não confirmada.'); }
  ...
  (async () => {
    try {
      ...
      controls(); await load();
      const link = new URLSearchParams(location.search), expenseId = link.get('expenseId'), allocationId = link.get('allocationId');
      if (active() && /^[1-9]\d*$/.test(expenseId || '') && positive(Number(expenseId)) && Number(expenseId) <= 2147483647) {
        await openDetail(Number(expenseId));
        if (active() && detail?.id === Number(expenseId)) {
          const row = /^[1-9]\d*$/.test(allocationId || '') ? el('allocationRows').querySelector('[data-allocation-id="' + Number(allocationId) + '"]') : null;
          (row || el('expenseDetail')).scrollIntoView({ block: 'start' });
        }
      }
