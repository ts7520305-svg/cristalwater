(function (root, factory) {
  ...
  async function preview(p, hash) {
    ...
    const duration = d.workTime ? (Date.parse(d.workTime.endedAt) - Date.parse(d.workTime.startedAt)) / 1000 : null;
    if (p.durationSeconds !== duration || duration !== null && (!Number.isSafeInteger(duration) || duration <= 0 || Date.parse(d.workTime.endedAt) > Date.parse(s.completedAt))) fail();
    return p;
  }
