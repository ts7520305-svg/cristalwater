(function (factory) {
  ...
  async function verify(p, hash, allocation) {
    ...
    const f = facts(p);
    if (p.materials.schema === 2) {
      const rules = typeof module === 'object' && module.exports ? require('./cw-equipment-material-review-rules') : globalThis.CWEquipmentMaterialReviewRules;
      const proof = p.materialRevision, e = proof?.revision, d = p.consumptionSource.declarations.find(row=>row.id===p.completionId);
      await rules.preview(e?.preview, hash);
      if (!e || e.schema!==1 || !rules.uuid(e.id) || !/^ADMIN:[1-9]\d*$/.test(e.owner) || !iso(e.createdAt) || typeof e.reason!=='string' || e.reason.trim()!==e.reason || e.reason.length<3 || e.reason.length>500 || e.completionId!==p.completionId || !sha(proof.hash) || await hash(e)!==proof.hash || e.preview.schema!==2 || e.preview.proposed.action!=='REPLACE' || await hash(e.preview.proposed.record)!==p.materialsHash || e.preview.targetHash!==p.target.hash || e.preview.originReview.hash!==p.materials.originReviewHash || d.revision?.headHash!==proof.hash || d.revision?.action!=='REPLACE' || Date.parse(e.createdAt)<Date.parse(e.preview.originReview.revision.createdAt)) throw Error('Reveja a origem histórica e a declaração administrativa dos materiais.');
    } else if (Object.hasOwn(p,'materialRevision')) throw Error('A prova histórica não corresponde aos materiais selecionados.');
