(function () {
  ...
  const key = (session, day) => 'cwLegacyRoute:v3:' + session.owner + ':' + day;
  ...
  function read(session, day) {
    ...
    if (!raw) {
      if (localStorage.getItem(key(session, day).replace('cwLegacyRoute:v3:', 'cwLegacyRoute:v2:'))) throw Error('A rota antiga pode estar incompleta. Foi preservada; consulte a rota completa com rede antes de trabalhar offline.');
