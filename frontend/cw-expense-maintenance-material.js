(function () {
  ...
  window.CWExpenseMaintenanceMaterial = { create(host) {
    ...
    let revision = 0, stamp = '', selected = null, completionId = null, preview = null, voiding = null, page = 1, total = 0;
    ...
    const draftKey = () => voiding ? key(voiding.share.expenseId, voiding.share.allocationId, voiding.share.id, 'void') : selected && completionId ? key(context().expense.id, selected.id, completionId, 'share') : null;
    ...
    function picker(visible) { el('maintenanceMaterialCandidates').hidden = !visible; el('maintenanceMaterialPrevious').hidden = !visible; el('maintenanceMaterialNext').hidden = !visible; el('maintenanceMaterialChange').hidden = visible || !!voiding; el('maintenanceMaterialQuantityBox').hidden = visible || !!voiding; el('maintenanceMaterialForm').hidden = visible; }
    function clear() { revision++; selected = preview = voiding = completionId = null; stamp = ''; page = 1; total = 0; el('maintenanceMaterialBox').hidden = true; el('maintenanceMaterialCandidates').replaceChildren(); el('maintenanceMaterialFacts').replaceChildren(); el('maintenanceMaterialStatus').textContent = ''; el('maintenanceMaterialForm').reset(); el('maintenanceMaterialQuantity').value = ''; picker(true); }
    function controls() {
      const c = context(), writable = c.canWrite && stamp === c.stamp;
    ...
    function showFacts(p, removing = false) {
      ...
      node(box, 'p', 'Já repartido nesta revisão, em todas as compras: ' + p.maintenanceUsed.quantity + ' ' + p.material.unit + ' · ' + p.monthRef + ' (UTC)');
      ...
      if (!removing) node(box, 'p', 'Fica na visita: ' + money(p.remainingAmountCents) + ' / ' + p.remainingQuantity + ' ' + p.material.unit + ' · Ainda sem parcela nesta revisão: ' + p.remainingMaintenanceQuantity + ' ' + p.material.unit);
    ...
    async function calculate() {
      ...
      try {
        ...
        const result = await host.request('/' + c.expense.id + '/maintenance-material-preview?' + new URLSearchParams({ allocationId: a.id, completionId: id, ...(raw ? { quantity: raw } : {}) }));
        ...
        if (p.expenseId !== c.expense.id || p.expenseVersion !== c.expense.version || p.allocationId !== a.id || p.completionId !== id || raw && quantity(raw) !== quantity(p.quantity)) throw Error('A parcela não corresponde à revisão selecionada.');
        if (!active() || rev !== revision || stamp !== context().stamp || inputQuantity() !== raw) return;
    ...
    async function candidates() {
      ...
      try {
        const result = await host.request('/' + c.expense.id + '/maintenance-material-candidates?allocationId=' + selected.id + '&page=' + page), data = result.candidates;
    ...
    el('maintenanceMaterialForm').addEventListener('submit', event => {
      ...
      if (preview) void host.execute(commands[0], { allocationId: preview.allocationId, completionId: preview.completionId, quantity: preview.quantity, amountCents: preview.amountCents, previewHash: preview.hash, reason, confirmed: true });
