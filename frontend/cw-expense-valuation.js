(function () {
  ...
  window.CWExpenseValuation = { create(host) {
    ...
    function controls() {
      ...
      const unavailable = value => (equipment && value !== 'MANUAL' || reminder && value === 'MATERIAL') || (repair || reminder) && (value === 'LABOR' && (c.expense?.sourceType !== 'MANUAL' || c.expense?.category !== 'LABOR' || !c.expense?.laborBasis && !distribution.has()) || value === 'MATERIAL' && (c.expense?.sourceType !== 'STOCK_PURCHASE' || host.target()?.snapshot?.materialMode === 'NONE'));
    ...
    async function validatePreview(p) {
      ...
      if (['REPAIR','MAINTENANCE_REMINDER'].includes(p.targetType)) { if (p.kind === 'LABOR') await work.preview(p,t,c.expense); else { repairSource(p.source, t.snapshot); if (p.source.item?.id !== p.purchaseItemId || p.source.item?.purchaseId !== c.expense.stockPurchaseId) throw Error('A compra não corresponde à reparação revista.'); } if (p.monthRef !== t.snapshot.endAt.slice(0, 7)) throw Error('O mês não corresponde à execução revista.'); }
      return p;
    }
    ...
    async function verify(result, record) {
      ...
      if (['VALUE_MATERIAL','VALUE_LABOR'].includes(e.command)) {
        ...
        if (['REPAIR','MAINTENANCE_REMINDER'].includes(a.targetType)) { if (d.kind === 'LABOR') await work.receipt(a,d,e.expenseId); else repairSource(a.valuationSnapshot.source, a.targetSnapshot); if (a.clientId !== a.valuationSnapshot.source.service.clientId || a.monthRef !== a.targetSnapshot.endAt.slice(0, 7)) throw Error('O destinatário ou mês do custo do serviço não está confirmado.'); }
      }
    }
