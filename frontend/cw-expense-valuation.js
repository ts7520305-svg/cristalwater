(function () {
  ...
  window.CWExpenseValuation = { create(host) {
    ...
    function signature() { const c = host.context(), t = host.target(); return JSON.stringify([c.epoch, c.detailEpoch, c.expense?.id, c.expense?.version, t?.type, t?.id, t?.hash, kind(), el('valuationLot').value, kind() === 'MATERIAL' ? el('valuationQuantity').value : '', el('allocationAmount').value, el('allocationMonth').value]); }
    function draft() {
      if (!active() || !editingId || host.context().expense?.id !== editingId) return;
      try { sessionStorage.setItem(key(), JSON.stringify({ kind: kind(), lot: el('valuationLot').value, quantity: el('valuationQuantity').value, labor: Object.fromEntries(laborFields.map(id => [id, el(id).value || (id === 'laborTechnician' ? wantedTechnician : '')])) })); } catch { note('O rascunho da valorização não pôde ser guardado.'); }
    }
    function invalidate(message = 'Calcule o custo com o serviço e as fontes atuais antes de confirmar.') {
      revision++; preview = null; confirmedSelection = ''; el('valuationPreview').replaceChildren(); el('valuationStatus').textContent = kind() === 'MANUAL' ? '' : message; el('allocationConfirmed').checked = false;
    ...
    function controls() {
      ...
      el('laborTechnician').disabled = !c.canWrite || !technicians.length;
    }
    function clear() {
      draft(); revision++; techRevision++; preview = null; confirmedSelection = ''; editingId = null; wantedTechnician = ''; technicians = [];
    ...
    function render(expense) {
      revision++; techRevision++; editingId = expense.id; preview = null; confirmedSelection = ''; technicians = []; el('laborBasisForm').reset(); el('laborTechnician').replaceChildren(); el('valuationLot').replaceChildren(); el('valuationKind').value = 'MANUAL'; el('valuationQuantity').value = '';
      ...
      el('laborBasisStatus').textContent = b ? b.technician.name + ' · ' + b.periodStart + ' a ' + b.periodEnd + ' · ' + b.paidMinutes + ' minutos pagos · Total do documento: ' + money(expense.amountCents) + '. Cada serviço usa o seu tempo medido.' : 'Ainda não há uma base de custo do trabalho confirmada.';
    ...
    async function calculate() {
      ...
      try {
        ...
        node(el('valuationPreview'), 'p', value.label); node(el('valuationPreview'), 'strong', money(value.amountCents));
        node(el('valuationPreview'), 'p', kind() === 'MATERIAL' ? 'Quantidade confirmada: ' + value.quantity + ' ' + value.quantityUnit + '. Base: ' + money(value.calculation.baseAmountCents) + ' para ' + value.calculation.baseQuantity + ' ' + value.quantityUnit + '.' : 'Tempo medido: ' + Number(value.quantity).toLocaleString('pt-PT') + ' segundos. Base: ' + money(value.calculation.baseAmountCents) + ' para ' + (Number(value.calculation.baseQuantity) / 60).toLocaleString('pt-PT') + ' minutos pagos.');
    ...
    function submit(reason) {
      ...
      const p = preview;
      void host.execute(p.kind === 'MATERIAL' ? 'VALUE_MATERIAL' : 'VALUE_LABOR', { kind: p.kind, targetType: p.targetType, targetId: p.targetId, targetHash: p.targetHash, monthRef: p.monthRef, purchaseItemId: p.purchaseItemId, quantity: p.quantity, amountCents: p.amountCents, valuationHash: p.valuationHash, previewHash: p.hash, reason, confirmed: true });
      return true;
    }
    ...
    function observe() { if (preview && confirmedSelection !== signature()) invalidate('A seleção mudou. Calcule novamente antes de confirmar.'); }
