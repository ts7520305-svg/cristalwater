(function () {
  ...
  window.CWExpenseValuation = { create(host) {
    ...
    function controls() {
      const c = host.context(), repair = el('allocationType').value === 'REPAIR', reminder = el('allocationType').value === 'MAINTENANCE_REMINDER', equipment = el('allocationType').value === 'MAINTENANCE_EQUIPMENT';
      const unavailable = value => (equipment && value !== 'MANUAL') || (repair || reminder) && (value === 'LABOR' && (c.expense?.sourceType !== 'MANUAL' || c.expense?.category !== 'LABOR' || !c.expense?.laborBasis && !distribution.has()) || value === 'MATERIAL' && (c.expense?.sourceType !== 'STOCK_PURCHASE' || host.target()?.snapshot?.materialMode === 'NONE'));
    ...
    function render(expense) {
      ...
      el('laborBasisStatus').textContent = b ? b.technician.name + ' · ' + b.periodStart + ' a ' + b.periodEnd + ' · ' + b.paidMinutes + ' minutos pagos · Total do documento: ' + money(expense.amountCents) + '. As visitas usam o tempo registado; as reparações e os lembretes usam intervalos próprios declarados e confirmados.' : 'Ainda não há uma base de custo do trabalho confirmada.';
    ...
    async function validatePreview(p) {
      ...
      if (['REPAIR','MAINTENANCE_REMINDER'].includes(p.targetType)) { if (p.kind === 'LABOR') await work.preview(p,t,c.expense); else { if(p.targetType==='MAINTENANCE_REMINDER'){await window.CWReminderMaterialRules.source(p.source,t.snapshot,host.hash);window.CWReminderMaterialRules.calculation(p.calculation,p.source);}else repairSource(p.source, t.snapshot); if (p.source.item?.id !== p.purchaseItemId || p.source.item?.purchaseId !== c.expense.stockPurchaseId) throw Error('A compra não corresponde ao serviço revisto.'); } if (p.monthRef !== t.snapshot.endAt.slice(0, 7)) throw Error('O mês não corresponde à execução revista.'); }
      return p;
    }
    async function calculate() {
      ...
      if (!t || !['REGULAR','EXTRA','REPAIR','MAINTENANCE_REMINDER'].includes(t.type)) { el('valuationStatus').textContent = 'Escolha e reveja um serviço com execução confirmada.'; return; }
      ...
      if (kind() === 'LABOR' && ['REPAIR','MAINTENANCE_REMINDER'].includes(t.type)) { const row = work.selection(); if (!row) { el('valuationStatus').textContent = 'Consulte e escolha um intervalo declarado e confirmado.'; return; } query.workIntervalId = String(row.id); }
      try {
        ...
        node(el('valuationPreview'), 'p', ['REPAIR','MAINTENANCE_REMINDER'].includes(value.targetType) && value.kind === 'LABOR' ? 'Intervalo declarado #' + value.workIntervalId + ' · ' + value.source.workInterval.snapshot.technicianName : value.label); node(el('valuationPreview'), 'strong', money(value.amountCents));
        if(value.source?.version===11&&value.targetType==='MAINTENANCE_REMINDER'&&value.kind==='MATERIAL'){const r=window.CWReminderMaterialRules,m=value.source.movements,consumed=m.filter(x=>x.movementType==='CONSUMPTION').reduce((q,x)=>q+r.quantity(x.quantity),0n),returned=m.filter(x=>x.movementType==='RETURN').reduce((q,x)=>q+r.quantity(x.quantity),0n);node(el('valuationPreview'),'p','Consumo original: '+r.decimal(consumed)+' '+value.quantityUnit+' · Devolvido: '+r.decimal(returned)+' · Consumo restante: '+r.decimal(consumed-returned)+'.');node(el('valuationPreview'),'p','As devoluções conservam os comprovativos originais. Custos anteriores permanecem reservados até à anulação explícita.');}
        node(el('valuationPreview'), 'p', kind() === 'MATERIAL' ? 'Quantidade confirmada: ' + value.quantity + ' ' + value.quantityUnit + '. Base: ' + money(value.calculation.baseAmountCents) + ' para ' + value.calculation.baseQuantity + ' ' + value.quantityUnit + '.' : (['REPAIR','MAINTENANCE_REMINDER'].includes(value.targetType) ? 'Tempo declarado e confirmado: ' : 'Tempo medido: ') + Number(value.quantity).toLocaleString('pt-PT') + ' segundos. Base: ' + money(value.calculation.baseAmountCents) + ' para ' + (Number(value.calculation.baseQuantity) / 60).toLocaleString('pt-PT') + ' minutos pagos.');
    ...
    function showSummary(value) {
      ...
      el('valuationSummaryBasis').textContent = 'Incluídos nos montantes atribuídos acima, pelo mês de conclusão do serviço. ' + value.count + ' valorizações · ' + value.reviewCount + ' por rever. Cobertura parcial: consumos, tempos registados nas visitas e intervalos próprios declarados de reparações e lembretes com base de custo confirmada.';
    }
    async function verify(result, record) {
      ...
      if (['VALUE_MATERIAL','VALUE_LABOR'].includes(e.command)) {
        ...
        if (['REPAIR','MAINTENANCE_REMINDER'].includes(a.targetType)) { if (d.kind === 'LABOR') await work.receipt(a,d,e.expenseId); else if(a.targetType==='MAINTENANCE_REMINDER'){await window.CWReminderMaterialRules.source(a.valuationSnapshot.source,a.targetSnapshot,host.hash);window.CWReminderMaterialRules.calculation(c,a.valuationSnapshot.source);if(a.activeMeasurementKey!==null||await host.hash(c)!==await host.hash(a.valuationSnapshot.calculation))throw Error('O cálculo original do custo mudou.');}else repairSource(a.valuationSnapshot.source, a.targetSnapshot); if (a.clientId !== a.valuationSnapshot.source.service.clientId || a.monthRef !== a.targetSnapshot.endAt.slice(0, 7)) throw Error('O destinatário ou mês do custo do serviço não está confirmado.'); }
      }
    }
