(() => {
  ...
  const style=document.createElement('style');style.textContent='#servicePanel{overflow-wrap:anywhere;background:var(--ds-surface,#fff);color:var(--ds-text,#172a35)}#servicePanel label,#servicePanel legend,#servicePanel h3,#servicePanel p{color:var(--ds-text,#172a35)}#servicePanel fieldset{min-width:0;border:1px solid var(--cw-border,#cbd5e1);border-radius:10px;margin:14px 0;padding:12px}#servicePanel legend{max-width:100%}#servicePanel label{display:block;min-width:0}#servicePanel input,#servicePanel select,#servicePanel textarea{max-width:100%;min-width:0;width:100%;font:inherit;color:var(--ds-text,#172a35);-webkit-text-fill-color:var(--ds-text,#172a35);background:var(--ds-surface,#fff);opacity:1;border:1px solid var(--cw-border,#cbd5e1);border-radius:8px;padding:8px;min-height:44px}#servicePanel select{appearance:none;overflow:hidden;text-overflow:ellipsis;padding-right:32px;background-image:linear-gradient(45deg,transparent 50%,currentColor 50%),linear-gradient(135deg,currentColor 50%,transparent 50%);background-position:calc(100% - 16px) 50%,calc(100% - 11px) 50%;background-size:5px 5px;background-repeat:no-repeat}#servicePanel .serviceSelection{display:block;margin:4px 0;overflow-wrap:anywhere}#servicePanel .serviceSelection[hidden]{display:none}#servicePanel button{max-width:100%;white-space:normal}#servicePanel textarea{min-height:80px}.serviceGrid{display:flex;flex-wrap:wrap;gap:12px}.serviceGrid>label{flex:1 1 160px}.serviceSlot{display:flex;flex-wrap:wrap;gap:8px;align-items:end}.serviceSlot>label{flex:1 1 100px}#serviceImpact{white-space:pre-line}#serviceImpact ul{padding-left:22px}';document.head.append(style);
  ...
  function addRule(season,value={}){
    ...
    const cadence=()=>{const weekly=row.querySelector('[data-rule=frequency]').value==='WEEKLY',interval=Number(row.querySelector('[data-rule=interval]').value),anchor=row.querySelector('[data-rule=anchorOn]');row.querySelector('[data-rule=interval]').max=weekly?'52':'24';row.querySelector('.serviceAnchor').hidden=interval<=1;anchor.required=interval>1;anchor.disabled=interval<=1;row.querySelector('.serviceCadenceHelp').textContent=interval>1?`Visitas a cada ${interval} ${weekly?'semanas':'meses'}. A semana de referência começa à segunda-feira. Só há visitas a partir da data indicada, na semana ou mês ativo; a cadência continua entre épocas e anos. A mensalidade mantém o valor acordado.`:`${weekly?'Visitas todas as semanas':'Visitas todos os meses'}, sem multiplicar a mensalidade.`;};
