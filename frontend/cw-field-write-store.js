(function () {
  ...
  const scopes = ['VISIT_PHOTO', 'VISIT_COMPLETION', 'TECHNICIAN_ALERT', 'EXTRA_VISIT_START', 'EXTRA_VISIT_PHOTO', 'EXTRA_VISIT_COMPLETION', 'EXTRA_VISIT_CORRECTION'];
  ...
  async function validate(record, captured, bytes = false) {
    ...
    if(record.reviewedAt && (record.scope!=='EXTRA_VISIT_CORRECTION'||record.response?.applied!==false||!Number.isFinite(Date.parse(record.reviewedAt))))throw Error('Revisão guardada inválida. Preserve os dados.');
  ...
  async function acknowledgeRejection(requestId,captured=session()){return locked(requestId,captured,async()=>{const row=await get(requestId,captured);if(row.scope!=='EXTRA_VISIT_CORRECTION'||row.response?.applied!==false)throw Error('Apenas uma recusa confirmada pode ser reconhecida.');await update(row,captured,current=>({...current,reviewedAt:current.reviewedAt||new Date().toISOString()}));window.dispatchEvent(new Event('cw:field-write-change'));});}
