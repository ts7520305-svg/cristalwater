(function () {
  ...
  const equipmentTimes = value => value?.intervals || (value ? [value] : []);
  function equipmentTime(value,open=false) {
    const fields=(v,keys)=>v&&typeof v==='object'&&!Array.isArray(v)&&Object.keys(v).length===keys.length&&keys.every(k=>Object.hasOwn(v,k));
    const multiple=value?.intervals!==undefined,times=equipmentTimes(value);
    if(multiple&&!fields(value,['intervals'])||!Array.isArray(times)||!times.length||times.length>20||times.some((w,i,a)=>!fields(w,['startAt','endAt'])||!workInstant(w.startAt)||(w.endAt===null?!(open&&i===a.length-1):!workInstant(w.endAt)||Date.parse(w.endAt)<=Date.parse(w.startAt))||i&&Date.parse(w.startAt)<Date.parse(a[i-1].endAt)))throw Error('Reveja os intervalos: até 20, por ordem, sem sobreposição e com início e fim válidos.');
    return multiple?{intervals:times.map(w=>({startAt:w.startAt,endAt:w.endAt}))}:{startAt:value.startAt,endAt:value.endAt};
  }
  const workTime=value=>{try{equipmentTime(value);return true;}catch(_){return false;}};
  ...
  function equipmentMaterials(value) {
    const fields = (v, names) => v && typeof v === 'object' && !Array.isArray(v) && Object.keys(v).length === names.length && names.every(k => Object.hasOwn(v, k));
    const normalize = v => v.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase().replace(/[^A-Z0-9\s_-]/g, '').replace(/\s+/g, ' ').trim();
    const invalid = () => { throw Error('Confirme os materiais: produto, unidade e quantidade positiva até 100000, com no máximo seis casas decimais. Não repita produto e unidade.'); };
    if (!fields(value, ['mode', 'items']) || !['NONE', 'DECLARED'].includes(value.mode) || !Array.isArray(value.items) || value.items.length > 20 || (value.mode === 'NONE' ? value.items.length !== 0 : value.items.length === 0)) invalid();
    const seen = new Set();
    const items = value.items.map(item => {
      if (!fields(item, ['productName', 'unit', 'quantity']) || typeof item.productName !== 'string' || item.productName.length > 160 || typeof item.unit !== 'string' || item.unit.length > 24 || typeof item.quantity !== 'string' || !/^\d{1,12}(\.\d{1,6})?$/.test(item.quantity)) invalid();
      const [whole, fraction = ''] = item.quantity.split('.'), n = BigInt(whole) * 1000000n + BigInt(fraction.padEnd(6, '0'));
      const productName = normalize(item.productName), unit = normalize(item.unit), key = JSON.stringify([productName, unit]);
      if (!productName || !unit || n <= 0n || n > 100000000000n || seen.has(key)) invalid(); seen.add(key);
      return { productName, unit, quantity: (n / 1000000n).toString() + (n % 1000000n ? '.' + (n % 1000000n).toString().padStart(6, '0').replace(/0+$/, '') : '') };
    });
  ...
  function confirmation(response, record) {
    ...
    if (photoScope(record.scope)) {
      ...
      if (!response.applied) {
        ...
        const t = c.workTime;
        if(p.workTime){
          const input=equipmentTime(p.workTime),times=equipmentTimes(input),durationMs=times.reduce((n,w)=>n+Date.parse(w.endAt)-Date.parse(w.startAt),0),expected=input.intervals?{schema:2,basis:'DECLARED_EQUIPMENT_WORK_INTERVALS',intervals:times.map(w=>({...w,durationMs:Date.parse(w.endAt)-Date.parse(w.startAt)})),durationMs}:{schema:1,basis:'DECLARED_EQUIPMENT_WORK_INTERVAL',...input,durationMs};
          if(!t||!equal({...t,origin:undefined},expected)||t.origin?.visitId!==p.visitId||t.origin?.visitType!==p.visitType||t.origin?.poolId!==p.poolId||(record.owner.startsWith('ADMIN:')?!id(t.origin?.technicianId):t.origin?.technicianId!==Number(record.owner.split(':').at(-1)))||!workInstant(t.origin?.visitStartAt)||times.some(w=>Date.parse(w.startAt)<Date.parse(t.origin.visitStartAt)||Date.parse(w.endAt)>Date.parse(c.completedAt)))throw Error('O tempo da revisão ainda não está confirmado. Conserve o pedido original.');
        }else if(t!==undefined)throw Error('O tempo da revisão ainda não está confirmado. Conserve o pedido original.');
        const m = c.materials;
        if (Object.hasOwn(p, 'materials') ? !m || m.schema !== 1 || m.basis !== 'DECLARED_EQUIPMENT_MATERIALS' || Object.keys(m).length !== 5 || !equal({ mode: m.mode, items: m.items }, equipmentMaterials(p.materials)) || m.origin?.visitId !== p.visitId || m.origin?.visitType !== p.visitType || m.origin?.poolId !== p.poolId || !id(m.origin?.clientId) || Object.keys(m.origin).length !== 5 || (record.owner.startsWith('ADMIN:') ? !id(m.origin?.technicianId) : m.origin?.technicianId !== Number(record.owner.split(':').at(-1))) : m !== undefined) throw Error('Os materiais da revisão ainda não estão confirmados. Conserve o pedido original.');
      }
  ...
  async function validate(record, captured, bytes = false) {
    ...
    if(record.scope==='EQUIPMENT_MAINTENANCE' && (Object.keys(record.payload).some(field=>!['visitType','visitId','poolId','expectedVersion','notes','confirmed','workTime','materials'].includes(field)) || !['REGULAR','EXTRA'].includes(record.payload.visitType) || !id(record.payload.visitId) || !id(record.payload.poolId) || !id(record.payload.expectedVersion) || record.payload.confirmed!==true || typeof record.payload.notes!=='string' || record.payload.notes.trim().length<3 || record.payload.notes.length>3000 || Object.hasOwn(record.payload,'workTime') && !workTime(record.payload.workTime)))throw Error('Revisão guardada inválida. Preserve os dados.');
    if (record.scope === 'EQUIPMENT_MAINTENANCE' && Object.hasOwn(record.payload, 'materials')) equipmentMaterials(record.payload.materials);
  ...
  window.CWFieldWriteStore = { session, adminSession:()=>session(true), same, prepare, send, records, get, remove, flush, digest, hash, confirmation, acknowledgeRejection, equipmentMaterials, equipmentTime, equipmentTimes };
})();
