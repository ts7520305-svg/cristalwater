(function(root,factory){
  ...
  const intervals=d=>d?.workIntervals|| (d?.workTime?[d.workTime]:[]);
  function resourceInput(v){
    if(v?.workIntervals===undefined)return resources.input(v);
    if(!fields(v,['technicianId','materials','workIntervals'])||!Array.isArray(v.workIntervals)||v.workIntervals.length<1||v.workIntervals.length>20)fail();
    const d=resources.input({technicianId:v.technicianId,materials:v.materials,workTime:v.workIntervals[0]});
    for(const [i,w]of v.workIntervals.entries()){
      resources.input({technicianId:v.technicianId,materials:null,workTime:w});
      if(i&&Date.parse(w.startedAt)<Date.parse(v.workIntervals[i-1].endedAt))fail();
    }
    return {technicianId:d.technicianId,materials:d.materials,workIntervals:v.workIntervals};
  }
  function input(v){
    if(!fields(v,['action','recordId','data'])||!['DECLARE','VOID'].includes(v.action))fail();
    if(v.action==='VOID'){if(!uuid(v.recordId)||v.data!==null)fail();}
    else {if(v.recordId!==null)fail();const d=resourceInput(v.data);if(d.materials&&d.materials.items.some((item,n)=>['productName','unit','quantity'].some(k=>item[k]!==v.data.materials.items[n][k])))fail();}
    return v;
  }
  ...
  function compare(data,parent,peers,movements){
    resourceInput(data);if(!links.parent(parent)||data.technicianId!==parent.technicianId||!Array.isArray(peers)||!Array.isArray(movements))fail();
    ...
    return {materials:lines,durationSeconds:ownTimes.length?ownTimes.reduce((sum,w)=>sum+(Date.parse(w.endedAt)-Date.parse(w.startedAt))/1000,0):null};
  }
  async function preview(p,hash){
    if(!fields(p,['available','schema','basis','reminderId','selection','contextHash','previousHash','origin','association','parent','peers','movements','comparison','affectedShares',...(p?.affectedCosts!==undefined?['affectedCosts']:[]),'original','hash'])||p.available!==true||![1,2].includes(p.schema)||p.basis!==basis||!positive(p.reminderId)||!sha(p.contextHash)||p.previousHash!==null&&!sha(p.previousHash)||!sha(p.hash)||await hash(facts(p))!==p.hash)fail();input(p.selection);
    ...
    if(p.selection.action==='VOID'){
      ...
      if(!p.original.applied||p.schema!==p.original.event.preview.schema||p.original.event.id!==p.selection.recordId||p.previousHash!==p.original.eventHash||await hash(o)!==await hash(p.original.event.preview.origin)||['association','parent','peers','movements','comparison'].some(k=>p[k]!==null))fail();return p;
    }
    ...
    const ap=a.event.preview;if(!a.applied||p.schema!==(p.selection.data.workIntervals===undefined?1:2)||a.event.id!==o.associationId||await hash(p.parent)!==ap.parentHash||o.visitType!==p.parent.type||o.visitId!==p.parent.id||['clientId','poolId','technicianId'].some(k=>o[k]!==ap.origin[k])||await hash(resourceInput(p.selection.data))!==await hash(p.selection.data))fail();
    if(intervals(p.selection.data).some(w=>Date.parse(w.endedAt)>Date.parse(ap.source.completedAt)))fail();
