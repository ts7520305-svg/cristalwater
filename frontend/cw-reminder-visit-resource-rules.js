(function(root,factory){
  ...
  async function preview(p,hash){
    if(!fields(p,['available','schema','basis','reminderId','selection','contextHash','previousHash','origin','association','parent','peers','movements','comparison','affectedShares','original','hash'])||p.available!==true||p.schema!==1||p.basis!==basis||!positive(p.reminderId)||!sha(p.contextHash)||p.previousHash!==null&&!sha(p.previousHash)||!sha(p.hash)||await hash(facts(p))!==p.hash)fail();input(p.selection);
