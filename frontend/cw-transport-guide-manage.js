(function () {
  'use strict';
  const R = window.CWTransportManageRules, copy = window.CWTransportManageCopy;
  ...
  function clearFields() { $('consumeForm').reset(); $('transportFields').replaceChildren(); $('workGuide').replaceChildren(); $('confirm').checked = false; }
  function active() {
  ...
  async function request(path, job, config = {}) {
    const res = await fetch('/api/transport-guide-manage' + path, { ...config, headers: { Authorization: 'Bearer ' + actor.token, 'Content-Type': 'application/json' }, cache: 'no-store', redirect: 'error', signal: job.controller.signal });
    if (!current(job)) throw Error('stale');
    if ([401, 403].includes(res.status)) { invalid = true; active(); throw Error('session'); }
    const trusted = res.headers.get('x-cw-transport') === 'transport-manage-v1' && res.headers.get('x-cw-owner') === actor.owner && res.headers.get('cache-control') === 'private, no-store' && (res.headers.get('content-type') || '').startsWith('application/json');
  ...
  function collect() {
    const changes={},action=$('action').value;
    if(action==='EDIT')for(const k of R.fields){const mode=$('mode-'+k).value;if(mode==='keep')continue;
      if(mode==='clear'){changes[k]=null;continue;}
      if(k==='isDraft')changes[k]=mode==='draft';
      else if(k==='validFrom'||k==='validUntil'){const value=window.CWOnboardingRules.resolve($('value-'+k).value,$('time-'+k).value);if(!value)return null;changes[k]=value;}
      else changes[k]=$('value-'+k).value;
    }
    return R.input({guideId:target?.id,action,reason:$('notes').value,changes,workGuideId:action==='REOPEN'&&target.workGuides.length?R.id($('workGuide').value):null});
  }
  const fieldLabel=k=>k==='notes'?'guideNotes':k==='isDraft'?'draft':k;
  function displayValue(k,v){if(v===null)return t('missing');if(k==='isDraft')return t(v?'draftValue':'referenceValue');if(k==='validFrom'||k==='validUntil')return date(v)+' · '+v;return v===''?t('emptyValue'):String(v);}
  function buildFields(){
    for(const k of R.fields){const row=node('article','');row.className='hub-card';row.dataset.field=k;row.append(node('h4',''));
      const detail=node('p','');detail.dataset.current=k;row.append(detail);
      const label=node('label',''),caption=node('span',t('changeMode')),select=node('select','');caption.dataset.copy='changeMode';select.id='mode-'+k;
      const modes=k==='isDraft'?['keep','draft','referenced']:k==='validFrom'?['keep','set']:['keep','set','clear'];
      for(const mode of modes){const option=node('option',t('mode_'+mode));option.value=mode;option.dataset.copy='mode_'+mode;select.append(option);}select.addEventListener('change',render);label.append(caption,select);row.append(label);
      if(k!=='isDraft'){const label=node('label',''),span=node('span',t('newValue')),input=node(k==='notes'?'textarea':'input','');span.dataset.copy='newValue';input.id='value-'+k;
        if(k==='validFrom'||k==='validUntil'){input.type='datetime-local';input.step='60';input.value=target[k]?window.CWOnboardingRules.localMinute(target[k]):'';}else{if(k!=='notes')input.type='text';input.maxLength={codeAT:200,origin:500,destination:500,notes:2000}[k];input.value=target[k]??'';}label.append(span,input);label.dataset.value=k;row.append(label);
        if(k==='validFrom'||k==='validUntil'){const label=node('label',''),span=node('span',t('timeChoice')),select=node('select','');span.dataset.copy='timeChoice';select.id='time-'+k;for(const [value,key]of [['','normalTime'],['first','first'],['second','second']]){const option=node('option',t(key));option.value=value;option.dataset.copy=key;select.append(option);}label.append(span,select);label.dataset.value=k;row.append(label);}
      }
      $('transportFields').append(row);
    }
    $('workGuide').replaceChildren();const blank=node('option',t('chooseWork'));blank.value='';blank.dataset.copy='chooseWork';$('workGuide').append(blank);
    for(const w of target.workGuides){const option=node('option','#'+w.id+' · '+w.status+(w.closedAt?' · '+date(w.closedAt):''));option.value=w.id;$('workGuide').append(option);}
  }
  function choose(guide) {
    if(!active()||blocked()||busy()||pending||target||!data||!R.choice(guide))return;
    target=guide;review=proposal=receipt=null;clearFields();$('action').value=R.actions.includes(url.searchParams.get('action'))?url.searchParams.get('action'):'EDIT';buildFields();
  ...
  function render() {
    ...
    $('editFields').hidden=$('action').value!=='EDIT';$('reopenField').hidden=$('action').value!=='REOPEN'||!target?.workGuides.length;
    $('actionHint').textContent=t('hint_'+$('action').value);
    for(const option of $('workGuide').options){const w=target?.workGuides.find(w=>String(w.id)===option.value);if(w)option.textContent='#'+w.id+' · '+t('status_'+w.status)+' · '+(w.technician?.name||t('none'))+(w.closedAt?' · '+date(w.closedAt):'');}
    for(const row of $('transportFields').children){const k=row.dataset.field;row.querySelector('h4').textContent=t(fieldLabel(k));row.querySelector('[data-current]').textContent=t('currentValue')+': '+displayValue(k,target[k]);const set=$('mode-'+k).value==='set';for(const label of row.querySelectorAll('[data-value]')){label.hidden=!set;for(const input of label.querySelectorAll('input,textarea,select'))input.disabled=!set||blocked()||busy()||!!pending||!!review;}}
    $('reviewPanel').hidden=!review||blocked()||!!pending;$('reviewFacts').replaceChildren();$('reviewRows').replaceChildren();
    if(review){const p=review.proposal,c=review.choice,plan=review.plan,dl=$('reviewFacts');fact(dl,'plate',c.vehicle?.plate);fact(dl,'guide','#'+c.id+' — '+(c.codeAT||t('missing')));fact(dl,'action',t('action_'+p.action));fact(dl,'notes',p.reason);fact(dl,'status',t('status_'+c.status)+' → '+t('status_'+plan.status));fact(dl,'closeWorks',plan.closeWorkGuideIds.map(id=>'#'+id).join(', ')||t('none'));fact(dl,'reopenWork',plan.reopenWorkGuideId?'#'+plan.reopenWorkGuideId:t('none'));if(plan.reopenWorkGuideId){const tech=c.workGuides.find(w=>w.id===plan.reopenWorkGuideId)?.technician;fact(dl,'technician',tech?'#'+tech.id+' — '+tech.name:t('none'));}fact(dl,'resolveLocks',plan.resolveLockIds.map(id=>'#'+id).join(', ')||t('none'));
      for(const k of R.fields.filter(k=>plan.before[k]!==plan.after[k])){const card=node('article',''),facts=node('dl','');card.className='hub-card';card.append(node('h4',t(fieldLabel(k))));fact(facts,'beforeValue',displayValue(k,plan.before[k]));fact(facts,'afterValue',displayValue(k,plan.after[k]));card.append(facts);$('reviewRows').append(card);}}
    ...
    if(receipt){const dl=$('receiptFacts');fact(dl,'guide','#'+receipt.guideId);fact(dl,'confirmedAt',date(receipt.receipt.confirmedAt));fact(dl,'audit','#'+receipt.auditId);if(receipt.guide){fact(dl,'plate',receipt.guide.vehicle?.plate);fact(dl,'action',t('action_'+receipt.proposal.action));fact(dl,'status',t('status_'+receipt.guide.status));fact(dl,'closeWorks',receipt.plan.closeWorkGuideIds.map(id=>'#'+id).join(', ')||t('none'));fact(dl,'reopenWork',receipt.plan.reopenWorkGuideId?'#'+receipt.plan.reopenWorkGuideId:t('none'));}}
  ...
  try { actor = identity(); if (!actor) throw Error(); key = 'cw:transport-guide-manage:v1:' + actor.owner; document.body.dataset.requiredRole = actor.role; } catch (_) { invalid = true; }
  $('action').addEventListener('change',()=>{if(active())render();});
