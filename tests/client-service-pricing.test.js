...
describe('price presentation independently checked in the browser',()=>{
 const shared=require('../frontend/cw-client-service-pricing');
 const input=()=>({startsOn:'2032-01-01',endsOn:'2032-12-31',seasons:[{label:'Visitas',fromMonth:1,toMonth:6,billing:'PER_VISIT',visitAmount:'25.55',monthlyAmount:'0'},{label:'Mensalidade',fromMonth:7,toMonth:12,billing:'INCLUDED_MONTHLY',monthlyAmount:'150'}]});
 it('shows unit pricing and the monthly amount separately, with no assumed completed visits',()=>{const p=shared.fromInput(input()),june=shared.calculate(p,'2032-06'),july=shared.calculate(p,'2032-07');expect(june.amount).toBe(0);expect(june.perVisitRates).toEqual([{period:'SEASON_1',label:'Visitas',unitAmount:25.55}]);expect(july.amount).toBe(150);expect(july.perVisitRates).toEqual([]);expect(june.variableCharges).toBe('CONFIRMED_COMPLETED_VISITS_ONLY');});
 it('calculates civil proration across leap February and free unit prices exactly',()=>{const i=input();i.startsOn='2032-02-15';i.seasons=[{label:'Mês',fromMonth:1,toMonth:6,monthlyAmount:'100'},{label:'Livre',fromMonth:7,toMonth:12,billing:'PER_VISIT',monthlyAmount:'0',visitAmount:'0'}];const p=shared.fromInput(i);expect(shared.calculate(p,'2032-02').amount).toBe(51.72);expect(shared.calculate(p,'2032-07').perVisitRates[0].unitAmount).toBe(0);});
 it('rejects altered simulation amounts, types and unexplained extra fields',()=>{const p=shared.fromInput(input());for(const change of [v=>v.perVisitRates[0].unitAmount=1,v=>v.amount=100,v=>v.variableCharges='PLANNED_VISITS',v=>v.futureRevenue=200]){const v=shared.calculate(p,'2032-06');change(v);expect(()=>shared.verify(v,p,'2032-06')).toThrow();}});
 it('rejects gaps, overlapping seasons and fractional cents before a confirmation',()=>{for(const value of [true,'','0.001',-1,Infinity])expect(()=>shared.cents(value)).toThrow();const p=shared.fromInput(input());p.seasons[1].fromMonth=6;expect(()=>shared.calculate(p,'2032-06')).toThrow();p.seasons[1].fromMonth=8;expect(()=>shared.calculate(p,'2032-06')).toThrow();});
});
